<?php
if ($func->m_bd->update("UPDATE " . $srv_settings['table_prefix'] . "questions SET subjectid=" . (int)$f_subjectid . ",eventid=" . (int)$f_subjectid . ", question_text=" . $f_question_text . ", question_points=" . $f_question_points . ", question_time=" . $f_question_time . ", question_type=" . $f_question_type . ", question_type2=" . $f_question_type2 . ", question_shufflea=" . $f_question_shufflea . " WHERE questionid=" . $f_questionid) === false)
    showDBError(__FILE__, 1);

if ($func->m_bd->update("DELETE FROM " . $srv_settings['table_prefix'] . "answers WHERE questionid=" . $f_questionid) === false)
...
for ($i = 1; $i <= count($f_answer_text); $i++) {
    if (!empty($f_answer_text[$i])) {
        ...
        if ($func->m_bd->insert($str_qry) === false)
