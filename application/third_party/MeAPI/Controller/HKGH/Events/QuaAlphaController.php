<?php
class MeAPI_Controller_HKGH_Events_QuaAlphaController implements MeAPI_Controller_HKGH_Events_QuaAlphaInterface {
    protected $_response;
    private $CI;
    private $url_service;
    ...
    public function __construct() {
        ...
        $this->CI->load->MeAPI_Model('Service/HKGH/QuaAlphaModel');
    ...
    public function index(MeAPI_RequestInterface $request){
        ...
        $this->CI->template->write_view('content', 'game/hkgh/Events/quaalpha/'.$_GET['view'].'/index', $this->data);
    ...
    public function add(MeAPI_RequestInterface $request){
        ...
        $this->data['url_service'] = $this->url_service;
        $this->CI->template->write_view('content', 'game/hkgh/Events/quaalpha/'.$_GET['view'].'/add', $this->data);
        $this->_response = new MeAPI_Response_HTMLResponse($request, $this->CI->template->getHTML());
    }
    public function edit(MeAPI_RequestInterface $request){
        ...
        $linkinfo = $this->url_service.'/cms/quaalpha/get_'.$_GET['view'].'?id='.$id;
    ...
    public function import(MeAPI_RequestInterface $request) {
        ...
        if ($this->CI->input->post()){
            ...
            if(!in_array($file[1],array('xls'))){
                ...
                $data_excel = array();
                for ($i=2;$i<=$rowsnum;$i++){
                    $data_excel[] = array('msi'=>$excel->val($i,1),'item_old'=>$excel->val($i,2),'item_new'=>$excel->val($i,3));
                }
                $this->CI->QuaAlphaModel->saveItem($data_excel);
                $this->data['success'] = 'Import thành công';
            }
        }
        $this->CI->template->write_view('content', 'game/hkgh/Events/quaalpha/'.$_GET['view'].'/import', $this->data);
        $this->_response = new MeAPI_Response_HTMLResponse($request, $this->CI->template->getHTML());
    }
    public function delete(MeAPI_RequestInterface $request) {
        $this->authorize->validateAuthorizeRequest($request, 0); 
        $id = $_GET['id'];
        $linkInfo = $this->url_service.'/cms/quaalpha/delete_'.$_GET['view'].'?id='.$id;
    ...
    public function history(MeAPI_RequestInterface $request){
        ...
        $this->data['url_service'] = $this->url_service;
        $this->CI->template->write_view('content', 'game/hkgh/Events/quaalpha/history', $this->data);
        $this->_response = new MeAPI_Response_HTMLResponse($request, $this->CI->template->getHTML());
    }
    public function excel(MeAPI_RequestInterface $request){
        $this->authorize->validateAuthorizeRequest($request, 0);
        $linkInfo = $this->url_service.'/cms/quaalpha/excel?start='.$_GET['start'].'&end='.$_GET['end'];
        $j_items = file_get_contents($linkInfo);
