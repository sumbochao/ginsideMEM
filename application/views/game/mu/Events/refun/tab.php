...
<script>
    ...
    $(function () {
        $('#event').on('click', function (e) {
            window.location.href = "/?control=refun_mu&func=index&view=event&module=all";
        });
        $('#history').on('click', function (e) {
            window.location.href = "/?control=refun_mu&func=index&view=history&module=all";
        });
        $('#user').on('click', function (e) {
            window.location.href = "/?control=refun_mu&func=index&view=user&module=all";
        });
        $('#level').on('click', function (e) {
            window.location.href = "/?control=refun_mu&func=index&view=level&module=all";
        });
        $('#history_level').on('click', function (e) {
            window.location.href = "/?control=refun_mu&func=index&view=history_level&module=all";
        });
    });
</script>
<style>
...
<ul class="nav nav-tabs" role="tablist">
    ...
    <li role="presentation" <?php echo $_GET['view']=='history'?'class="active"':'';?>><a id="history" href="#history" aria-controls="history" role="tab" data-toggle="tab">LỊCH SỬ</a></li>
    <li role="presentation" <?php echo $_GET['view']=='user'?'class="active"':'';?> <?php echo $_SESSION['account']['id_group']==1?'style="display:block"':'style="display:none"';?>><a id="user" href="#user" aria-controls="user" role="tab" data-toggle="tab">IMPORT USER</a></li>
    <li role="presentation" <?php echo $_GET['view']=='level'?'class="active"':'';?> <?php echo $_SESSION['account']['id_group']==1?'style="display:block"':'style="display:none"';?>><a id="level" href="#level" aria-controls="level" role="tab" data-toggle="tab">IMPORT LEVEL</a></li>
    <li role="presentation" <?php echo $_GET['view']=='history_level'?'class="active"':'';?>><a id="history_level" href="#history_level" aria-controls="history_level" role="tab" data-toggle="tab">LỊCH SỬ LEVEL</a></li>
