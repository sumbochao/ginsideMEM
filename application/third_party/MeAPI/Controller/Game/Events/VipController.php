<?php
class MeAPI_Controller_Game_Events_VipController implements MeAPI_Controller_Game_Events_VipInterface {
    protected $_response;
    private $CI;
    private $url_service;
    ...
    public function index(MeAPI_RequestInterface $request){
        ...
        switch ($_GET['view']){
            case 'event':
                $this->data['title']= 'DANH SÁCH SỰ KIỆN VIP';
            break;
            case 'filters':
                $this->data['title']= 'DANH SÁCH QUÀ VIP';
            break;
        }
        $this->CI->template->write_view('content', 'game/Events/vip/'.$_GET['view'].'/index', $this->data);
    ...
    public function add(MeAPI_RequestInterface $request){
        ...
        switch ($_GET['view']){
            case 'event':
                $this->data['title']= 'THÊM SỰ KIỆN VIP';
            break;
            case 'filters':
                $this->data['title']= 'THÊM QUÀ VIP';
                $linkinfoslbEvent = $this->url_service.'/cms/vip/slbevent';
        ...
        $this->CI->template->write_view('content', 'game/Events/vip/'.$_GET['view'].'/add', $this->data);
        $this->_response = new MeAPI_Response_HTMLResponse($request, $this->CI->template->getHTML());
    }
    public function edit(MeAPI_RequestInterface $request){
        ...
        switch ($_GET['view']){
            case 'event':
                $this->data['title']= 'SỬA SỰ KIỆN VIP';
                break;
            case 'filters':
                $this->data['title']= 'SỬA QUÀ VIP';
                $linkinfoslbEvent = $this->url_service.'/cms/vip/slbevent';
                $infoDetailslbEvent = file_get_contents($linkinfoslbEvent);
                $slbEvent = json_decode($infoDetailslbEvent,true);
                $this->data['slbEvent'] = $slbEvent;
        ...
        $linkinfo = $this->url_service.'/cms/vip/gettem_'.$_GET['view'].'?id='.$id;
    ...
    public function delete(MeAPI_RequestInterface $request) {
        $this->authorize->validateAuthorizeRequest($request, 0); 
        $id = $_GET['id'];
        $linkInfo = $this->url_service.'/cms/vip/delete_'.$_GET['view'].'?id='.$id;
    ...
    public function history(MeAPI_RequestInterface $request){
        $this->authorize->validateAuthorizeRequest($request, 0);
        $this->data['title']= 'LỊCH SỬ VIP';
        $this->CI->template->write_view('content', 'game/Events/vip/history', $this->data);
        $this->_response = new MeAPI_Response_HTMLResponse($request, $this->CI->template->getHTML());
    }
    public function info(MeAPI_RequestInterface $request){
        $this->authorize->validateAuthorizeRequest($request, 0);
        $this->data['title']= 'THÔNG TIN VIP';
        $this->CI->template->write_view('content', 'game/Events/vip/info', $this->data);
        $this->_response = new MeAPI_Response_HTMLResponse($request, $this->CI->template->getHTML());
    }
    public function excel(MeAPI_RequestInterface $request){
        $this->authorize->validateAuthorizeRequest($request, 0);
        $linkInfo = $this->url_service.'/cms/vip/excel?game='.$_GET['game'].'&start='.strtotime($_GET['start']).'&end='.strtotime($_GET['end']);
        $j_items = file_get_contents($linkInfo);
	...
	public function excelinfo(MeAPI_RequestInterface $request){
        $this->authorize->validateAuthorizeRequest($request, 0);
        $linkInfo = $this->url_service.'/cms/vip/excelinfo?game='.$_GET['game'].'&start='.strtotime($_GET['start']).'&end='.strtotime($_GET['end']);
        $j_items = file_get_contents($linkInfo);
        $listItem = json_decode($j_items,true);
        $data .= '<html xmlns:x="urn:schemas-microsoft-com:office:excel">
        <head>
        ...
        if(count($listItem)>0){
            foreach($listItem as $v){
                ...
                $data .= '<tr>
                    ...
                    <td align="center">'.$v['facebook_uri'].'</td>
                    <td align="center">'.$create_date.'</td>
                </tr>';
                $i++;
            }
