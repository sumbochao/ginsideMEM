	...
	*/
	...
	public function saveItemi($arrParam,$option= NULL){
		...
		if($option['task']=='updatechannel'){
			...
			$arrData['path_folder_sign']    = $arrParam['path_folder_sign'];
			$this->_db_slave->where('id', $arrParam['id']);
            $this->_db_slave->update($this->_table,$arrData);
		}
    ...
    public function saveItem($arrParam,$option= NULL){
		...
		if($option['task']=='updatechannel'){
			...
			$arrData['ipa_name_sign']    = $arrParam['ipa_name_sign'];
			$arrData['path_folder_sign']    = $arrParam['path_folder_sign'];
			$arrData['current_sdk']    = isset($arrParam['current_sdk'])?$arrParam['current_sdk']:'';
			$this->_db_slave->where('id', $arrParam['id']);
            $this->_db_slave->update($this->_table,$arrData);
		}
    ...
    public function listGame(){
        ...
        $this->db_slave->select(array('id','app_fullname','service_id','service','type'));
	...
	/*
	 - hàm lấy id trong bảng tbl_sign_config_cert
	 nếu type_app là AppstoreDev
	 - trả về id của type_app là Appstore , cùng một Partner
	*/
	public function getCertIDWithPartnerAppDev($cert_id){
		$sql="select * from tbl_sign_config_cert where id=".$cert_id;
		$data=$this->_db_slave->query($sql);
		$rs=$data->row_array();
		$id_partner=$rs['idpartner'];
		//sau khi lấy dc idpartner, tiếp tục tìm id của bản Appstore cùng với partner vừa tìm dc
		$sqlid="select * from tbl_sign_config_cert where idpartner=".$id_partner." and cert_type='Appstore'";
		$dataid=$this->_db_slave->query($sqlid);
		$rsid=$dataid->row_array();
		$id=$rsid['id'];
		return $id; // đây chính là cert_id cần lọc của Appstore để lấy msv
	}
	//hàm lấy MeStoreVersion
	public function listMsvPlus($id_game,$type_app,$bunlde_name,$cert_id){
		// tim msv_id da public
		
		$id_cert=0;
		/*do msv không có loại AppstoreDev
		Nếu type_app là AppstoreDev , => sẽ lấy type_app là Appstore, cert_id lúc này sẽ lấy cert_id của bản AppStore
		*/
		if($type_app == "AppstoreDev"){
			//goi ham lay cert_id của bản Appstore
			$id_cert=$this->getCertIDWithPartnerAppDev($cert_id);
		}else{
			$id_cert=$cert_id;
		}
		//do msv không có bản AppstoreDev nên sẽ lấy msv thoe bản Appstore
		$type_app=$type_app=="AppstoreDev"?"Appstore":$type_app;
		
		$sql="select msv_id,SUBSTRING(msv_id,5) as MSVID from tbl_get_msv where service_id=".$id_game." and published in('yes','waiting') and type_app='".$type_app."' and platform='ios' and bunlderid='".$bunlde_name."' and cert_id=".$id_cert." ORDER BY SUBSTRING(msv_id,5) desc limit 1";
        ...
        $data = $this->_db_slave->select(array('*'))
						...
						->where('type_app',$type_app)
						->where('platform','ios')
						->where('bunlderid',$bunlde_name)
						->where('cert_id',$id_cert)
						->where('SUBSTRING(msv_id,5) >= '.$msvid)
                        ->from('tbl_get_msv')
                        ->get();
    ...
    public function getValueBundleID($arrParam){
        ...
        $data = $this->_db_slave->select(array('*'))
						...
						->where('bundleidentifier',$arrParam['bundleid'])
	...
	//them data into Plist files
	public function getKeyQueriesSchemes(){
        $data = $this->_db_slave->select(array('*'))
                        ->from("tbl_sign_ios_keys_queries_schemes")
                        ->get();
        if (is_object($data)) {
	...
	//update 08/01/2016
	public function listInfoPlist(){
        $this->_db_slave->select(array('*'));
        $this->_db_slave->from("tbl_sign_ios_keys_queries_schemes");
        $this->_db_slave->order_by('id','DESC');
        $data=$this->_db_slave->get();
        if (is_object($data)) {
            $result = $data->result_array();
	...
	public function checkexitskeyvalues($notes){
		$data = $this->_db_slave->select(array('*'))
                        ->from("tbl_sign_ios_keys_queries_schemes")
                        ->where('notes', $notes)
                        ->get();
	...
	public function addkeyvalues($param){
		$this->_db_slave->insert("tbl_sign_ios_keys_queries_schemes",$param);
		$id = $this->_db_slave->insert_id();
	}
	public function deletedkeyvalues($id){
		$this->_db_slave->delete("tbl_sign_ios_keys_queries_schemes",array('id' => $id)); 
	}
	public function GetListItem($param){
		$sql="select * from ".$this->_table." where id in($param)";
		$data1=$this->_db_slave->query($sql);
		$rs=$data1->result_array();
		return $rs;
	}
