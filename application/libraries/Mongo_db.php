...
class Mongo_db {
    ...
    public function switch_db($database = '') {
        ...
        try {
            ...
            throw new Exception("Unable to switch Mongo Databases: {$e->getMessage()}", 500);
        }
    }
    ...
    public function count($collection = "") {
        if (empty($collection))
            throw new Exception("In order to retreive a count of documents from MongoDB, a collection name must be passed", 500);
    ...
    public function add_index($collection = "", $keys = array(), $options = array()) {
        ...
        if ($this->db->{$collection}->ensureIndex($keys, $options) == TRUE):
            ...
            throw new Exception("An error occured when trying to add an index to MongoDB Collection", 500);
        endif;
    }
    ...
    public function remove_index($collection = "", $keys = array()) {
        ...
        if ($this->db->{$collection}->deleteIndex($keys, $options) == TRUE):
            ...
            throw new Exception("An error occured when trying to remove an index from MongoDB Collection", 500);
        endif;
    }
     ...
     * @return object
     */
    public function group($collection, $option) {
        try {
            $execute = $this->db->{$collection}->group($option['keys'], $option['initial'], $option['reduce'], $option['condition']);
            return $execute;
        } catch (MongoCursorException $exception) {
            throw new Exception('MongoDB command failed to execute: ' . $exception->getMessage(), 500);
        }
    ...
    private function connection_string() {
        ...
        //$this->persist = trim($config['mongo_persist'));
        //$this->persist_key = trim($config['mongo_persist_key'));
