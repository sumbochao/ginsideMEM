<?php
class functionAll extends CI_Controller
{
    public $m_bd;
    function __construct()
    {
        parent::__construct();
        $this->load->Model('m_tracnghiem','m_bd2');
        $this->m_bd = $this->m_bd2;
    }
    ...
    function writeErrorsWarningsBar()
    {
		...
		if (isset($g_vars['page']['errors']) && $g_vars['page']['errors'])
            $html.= '<p><b><font color="#cc0000">' . $g_vars['page']['errors'] . '</font></b>';
        if (isset($g_vars['page']['notifications']) && $g_vars['page']['notifications'])
            $html.= '<p><b><font color="#006000">' . $g_vars['page']['notifications'] . '</font></b>';
		return $html;
    }

    function getConfigItem($i_id,$srv_settings)
    {
        $i_result = NULL;
        $i_rSet1 = $this->m_bd->SelectLimit("SELECT config_value FROM " . $srv_settings['table_prefix'] . "config WHERE configid=" . (int)$i_id, 1);
        if ($i_rSet1) {
            foreach($i_rSet1 as $key=>$value) {
                $i_result = $value["config_value"];
            }
        }
        return $i_result;
    }
    ...
    {
        $i_recordcount = 0;
        $i_rSet1 = $this->m_bd->getRecordCount($i_table,$i_where);
        if ($i_rSet1) {
           $i_recordcount = (int)array_pop($i_rSet1);
        }
    ...
    {
        return $this->getTR(array($i_column1, $i_column2));
    }

    function writeTR2Fixed($i_column1, $i_column2)
    {
        return $this->getTR(array($i_column1, $i_column2), array(' width="20%"', ' width="80%"'));
    }
    ...
    function getSelectElement($i_name, $i_value, $i_values, $i_additional = '')
    {
        ...
        foreach ($i_values as $key => $option)
            $i_result .= '<option value="' . $this->convertTextValue($key) . '"' . ($key == $i_value ? " selected" : '') . '>' . $this->convertTextValue($option) . '</option>';
        $i_result .= '</select>';
        return $i_result;
    }
    ...
    function getTextEditor($i_type, $i_name, $i_value, $i_rows = 15, $i_cols = 50, $i_addon = '')
    {
        ...
        switch ($i_type) {
            case 0:
                $i_value = $this->convertTextAreaHTML(false, $i_value);
                return '<textarea name=' . $i_name . ' cols=' . $i_cols . ' rows=' . $i_rows . ' style="width: 99%;">' . $this->convertTextValue($i_value) . '</textarea>';
            case CONFIG_CONST_htmlareaeditor:
            ...
            case CONFIG_CONST_iseditor:
                ...
                break;
            case CONFIG_CONST_iseditor2:
                return '<textarea name=' . $i_name . ' cols=' . $i_cols . ' rows=' . $i_rows . ' id=' . $i_name_nospecialchars . '>' . $this->convertTextValue($i_value) . '</textarea><script language=JavaScript type="text/javascript">var ' . $i_name_nospecialchars . 'Editor = new InnovaEditor("' . $i_name_nospecialchars . 'Editor"); ' . $i_name_nospecialchars . 'Editor.width="100%"; ' . $i_name_nospecialchars . 'Editor.cmdAssetManager="modalDialogShow(\'' . $srv_settings['url_root'] . 'editors/iseditor2/assetmanager/assetmanager.php?lang=' . $lngstr['language_long'] . '\', 640, 465)"; ' . $i_name_nospecialchars . 'Editor.btnFlash=true; ' . $i_name_nospecialchars . 'Editor.btnMedia=true; ' . $i_name_nospecialchars . 'Editor.btnStyles=true; ' . $i_name_nospecialchars . 'Editor.css="shared/style.css"; ' . $i_name_nospecialchars . 'Editor.btnStrikethrough=true; ' . $i_name_nospecialchars . 'Editor.btnSuperscript=true; ' . $i_name_nospecialchars . 'Editor.btnSubscript=true; ' . $i_name_nospecialchars . 'Editor.btnLTR=true; ' . $i_name_nospecialchars . 'Editor.btnRTL=true; ' . $i_name_nospecialchars . 'Editor.btnClearAll=true; ' . $i_name_nospecialchars . 'Editor.btnPasteText=true; ' . $i_addon . ' ' . $i_name_nospecialchars . 'Editor.REPLACE("' . $i_name_nospecialchars . '");</script>';
                break;
        }
    }
    ...
    {
        return '<textarea name=' . $i_name . ' cols=' . $i_cols . ' rows=' . $i_rows . ' title="' . $this->convertTextValue($hint) . '">' . $this->convertTextValue($i_value) . '</textarea>';
    }

    function getPasswordBox($i_name, $i_value, $size = 50, $maxlength = 0)
    {
        return '<input name="' . $i_name . '" value="' . $this->convertTextValue($i_value) . '" class=inp type=password size=' . $size . (!empty($maxlength) ? " maxlength=$maxlength" : "") . '>';
    }
    ...
    {
        return '<input name="' . $i_name . '" value="' . $this->convertTextValue(@$i_value) . '" class=inp type=' . $i_type . ' size=' . $size . ($maxlength > 0 ? " maxlength=$maxlength" : "") . $i_additional . '>';
    }
    ...
    {
        return getInputElement($i_name, $i_value, $size, $maxlength, $i_additional);
    }

    function getBar($i_state, $i_content)
    {
        return '<table cellpadding=0 cellspacing=0 border=0 width="100%" style="border-left: 1px solid #E0E7F6; border-top: 1px solid #E0E7F6; border-right: 1px solid #CFD6E3; border-bottom: 1px solid #CFD6E3;"><tr><td colspan=3 height=1 style="background: #ffffff"><img src="/events/p_tracnghiem/images/1x1.gif" width=1 height=1></td></tr><tr style="background: url(images/barsmall-' . $i_state . '-background.gif) repeat-x;"><td width=1 style="background: #ffffff"><img src="/events/p_tracnghiem/images/1x1.gif" width=1 height=24></td><td align=center>' . $i_content . '</td><td width=1 style="background: #ffffff"><img src="/events/p_tracnghiem/images/1x1.gif" width=1 height=24></td></tr><tr><td colspan=3 height=1 style="background: #ffffff"><img src="/events/p_tracnghiem/images/1x1.gif" width=1 height=1></td></tr></table>';
    }
    ...
    function getMenu2Items($section_name,$lngstr)
    {
        global  $g_vars;
        $i_items = array();
        switch ($section_name) {
            ...
            case 'questionbank':
                //if ($G_SESSION['access_questionbank'] > 0)
                    $i_items['questionbank'] = array('?control=mobo_event_tracnghiem&func=question_bank', $lngstr['page_header_questionbank']);
                //if ($G_SESSION['access_subjects'] > 0)
                    $i_items['subjects'] = array('?control=mobo_event_tracnghiem&func=subjects', $lngstr['page_header_subjects']);
                break;
            case 'testmanager':
                //if ($G_SESSION['access_testmanager'] > 0)
                    $i_items['testmanager'] = array('?control=mobo_event_tracnghiem&func=test_manager', $lngstr['page_header_edittests']);
                //if ($G_SESSION['access_gradingsystems'] > 0)
                    $i_items['grades'] = array('?control=mobo_event_tracnghiem&func=grades', $lngstr['page_header_grades']);
                if (($g_vars['page']['selected_tab'] == 'editquestions') || ($g_vars['page']['selected_tab'] == 'editquestions-2') || ($g_vars['page']['selected_tab'] == 'testmanager-10'))
                   // if ($G_SESSION['access_testmanager'] > 0)
                        $i_items['editquestions'] = array('?control=mobo_event_tracnghiem&func=test_manager&action=editt&testid=' . (int)readGetVar('testid'), $lngstr['page_header_test_questions']);
                break;
            case 'reportsmanager':
                //if ($G_SESSION['access_reportsmanager'] > 0)
                    $i_items['reportsmanager'] = array('?control=mobo_event_tracnghiem&func=reports_manager', $lngstr['page_header_results']);


                if (($g_vars['page']['selected_tab'] == 'reportsmanager-2') || ($g_vars['page']['selected_tab'] == 'reportsmanager-3'))
                    //if ($G_SESSION['access_reportsmanager'] > 0)
                        $i_items['reportsmanager-2'] = array('?control=mobo_event_tracnghiem&func=reports_manager&action=viewq&resultid=' . (int)readGetVar('resultid'), $lngstr['page_header_results_questions']);
                break;
            case 'administration':
               ...
               // if ($G_SESSION['access_reporttemplates'] > 0)
                ...
                //sif ($G_SESSION['access_visitors'] > 0)
    ...
    function writePanel2(&$g_vars, $i_items)
    {
        global $lngstr, $srv_settings;
        $html= '<p><table cellpadding=0 cellspacing=0 border=0 width="100%"><tr><td>';
        $html.= '<table cellpadding=0 cellspacing=0 border=0>';
        $html.= '<tr><td width=3><img src="/events/p_tracnghiem/images/1x1.gif" width=3 height=1></td>';
        //$this->reset($i_items);
        if (list($key, $val) = each($i_items))
            $html.= '<td>' . $this->getBarSmall($g_vars['page']['selected_tab'] == $key ? 1 : 0, '&nbsp;&nbsp;<b>' . ($g_vars['page']['selected_tab'] == $key ? $val[1] : '<a class=bar2 href="' . $val[0] . '">' . $val[1] . '</a>') . '</b>&nbsp;&nbsp;') . '</td>';
        while (list($key, $val) = each($i_items)) {
            $html.= '<td width=5><img src="/events/p_tracnghiem/images/1x1.gif" width=5 height=1></td>';

            $html.= '<td>' . $this->getBarSmall($g_vars['page']['selected_tab'] == $key ? 1 : 0, '&nbsp;&nbsp;<b>' . ($g_vars['page']['selected_tab'] == $key ? $val[1] : '<a class=bar2 href="' . $val[0] . '">' . $val[1] . '</a>') . '</b>&nbsp;&nbsp;') . '</td>';
        }
        $html.= '<td width=3><img src="/events/p_tracnghiem/images/1x1.gif" width=3 height=1></td></tr>';

        $html.= '</table>';
        $i_guide_page_url = implode('.', $g_vars['page']['location']);
        /*echo '</td><td width=53 align=right><a id="infobar_button" href="javascript:ShowInfoBar(' . (($this->readCookieVar('showinfobar', 'N') == 'Y') ? 'false' : 'true') . ');" title="' . $lngstr['tooltip_button'] . '"><img src="/events/p_tracnghiem/images/dialog-info-small.gif" width=24 height=24 border=0></a><img src="/events/p_tracnghiem/images/1x1.gif" width=5 height=1><a id="helpbar_button" href="guide/guide.php?language=' . $srv_settings['language'] . '&page=' . $i_guide_page_url . '" title="' . $lngstr['help_button'] . '" target="_blank"><img src="/events/p_tracnghiem/images/dialog-help-small.gif" width=24 height=24 border=0></a></td><td width=3><img src="/events/p_tracnghiem/images/1x1.gif" width=3 height=1></td></tr></table>';
        echo '<table cellpadding=0 cellspacing=0 border=0 width="100%" style="background: #CFD6E3;"><tr><td height=1><img src="/events/p_tracnghiem/images/1x1.gif" width=1 height=1></tr></table>';
        */
		return $html;
    }
    ...
    function writeInfoBar($i_content)
    {
		...
		$html = '';
        $i_isvisible = $this->readCookieVar('showinfobar', 'N') == 'Y';
        if ($i_content) {
            $html.= '<div id="infobar" style="display: ' . ($i_isvisible ? 'block' : 'none') . ';"><table cellpadding=0 cellspacing=0 border=0 width="97%" align=center style="border: 1px solid #8097D1; background-color: #F3F6FF;">';
            $html.= '<tr><td width=32 vAlign=top style="padding: 5px;"><img src="/events/p_tracnghiem/images/dialog-info.gif" width=32 height=32></td>';
            $html.= '<td width="100%" vAlign=middle style="padding: 5px;"><a href="javascript:ShowInfoBar(false);" title="' . $lngstr['tooltip_closebar'] . '"><img src="/events/p_tracnghiem/images/button-cross-infobar.gif" align=right width=20 height=20 border=0></a>';
            $html.= $i_content;
            $html.= '</td>';
            $html.= '</tr></table></div>';
        }
        ...
        foreach ($i_tablefields as $i_fieldno => $i_field) {
            ...
            if ($i_field[2]) {
                ...
                if ($i_order_no == $i_fieldno && !$i_direction)
                    $i_content .= '<img src="/events/p_tracnghiem/images/button-order-asc.gif" width=10 height=8>';
                else $i_content .= '<a href="' . $i_url . '&order' . $i_orderid . '=' . $i_fieldno . '&direction' . $i_orderid . '="><img src="/events/p_tracnghiem/vimages/button-order-asc-inactive.gif" width=10 height=8 border=0></a>';
                if ($i_order_no == $i_fieldno && $i_direction)
                    $i_content .= '<img src="/events/p_tracnghiem/images/button-order-desc.gif" width=10 height=8>';
                else $i_content .= '<a href="' . $i_url . '&order' . $i_orderid . '=' . $i_fieldno . '&direction' . $i_orderid . '=DESC"><img src="/events/p_tracnghiem/images/button-order-desc-inactive.gif" width=10 height=8 border=0></a>';
                $i_content .= '</nobr>';
            }
        ...
        foreach ($i_columns as $i_fieldno => $i_field) {
            ...
            if ($i_field[2]) {
                ...
                if ($i_orderno == $i_fieldno && !$i_direction)
                    $i_content .= '<img src="/events/p_tracnghiem/images/button-order-asc.gif" width=10 height=8>';
                else $i_content .= '<a href="' . $i_url . '&order' . $i_orderid . '=' . $i_fieldno . '&direction' . $i_orderid . '="><img src="/events/p_tracnghiem/images/button-order-asc-inactive.gif" width=10 height=8 border=0></a>';
                if ($i_orderno == $i_fieldno && $i_direction)
                    $i_content .= '<img src="/events/p_tracnghiem/images/button-order-desc.gif" width=10 height=8>';
                else $i_content .= '<a href="' . $i_url . '&order' . $i_orderid . '=' . $i_fieldno . '&direction' . $i_orderid . '=DESC"><img src="/events/p_tracnghiem/images/button-order-desc-inactive.gif" width=10 height=8 border=0></a>';
                $i_content .= '</nobr>';
    ...
    function readTestQuestion(&$g_vars, &$G_SESSION, $i_questionno, $i_questionid)
    {
    ...
    function addNewUser($arrUserData = array(), $arrGroupIDs = array(), $bForceCreation = false)
    {
        ...
        foreach ($arrUserData as $name => $value) {
            ...
            $i_names .= $name;
            $i_values .= $this->qstr($value, get_magic_quotes_gpc());
        }
    ...
    function qstr($s,$magic_quotes=false)
    {
        if (!$magic_quotes) {
            ...
            if ($this->replaceQuote[0] == '\\'){
                ...
                $s = $this->adodb_str_replace(array('\\',"\0"),array('\\\\',"\\\0"),$s);
    ...
    function getTestAnswerDetails($resultid, $output_format = 'html')
    {
        ...
        if (!$i_rSet7) {
            ...
            while (!$i_rSet7->EOF) {
                ...
                for ($i = 1; $i < $i_answerno; $i++) {
                    switch ($i_rSet7->fields['question_type']) {
                        ...
                        case QUESTION_TYPE_TRUEFALSE:
                            ...
                            $i_result_detailed_3_items[$i_questionno] .= '<tr><td><img src="/events/p_tracnghiem/images/button-checkbox-' . ($i_answers_correct[$i] ? '2' : ($i == $i_answers_given ? '4' : '0')) . '.gif" width=13 height=13> &nbsp; ' . $i_answers[$i] . '</tr>';
                            break;
                        case QUESTION_TYPE_MULTIPLEANSWER:
                            $i_answers_given = explode(QUESTION_TYPE_MULTIPLEANSWER_BREAK, $i_rSet7->fields['result_answer_text']);
                            $i_result_detailed_3_items[$i_questionno] .= '<tr><td><img src="/events/p_tracnghiem/images/button-checkbox-' . ($i_answers_correct[$i] ? '2' : (in_array($i, $i_answers_given) ? '4' : '0')) . '.gif" width=13 height=13> &nbsp; ' . $i_answers[$i] . '</tr>';
                            break;
                        case QUESTION_TYPE_FILLINTHEBLANK:
                ...
                $i_result_detailed_3_items[$i_questionno] .= '<tr><td><img src="/events/p_tracnghiem/images/1x1.gif" width=1 height=7></td></tr>';
    ...
    function checkTestAnswer(&$G_SESSION, $i_questionno, $i_questionid, $i_answers)
    {
        global $g_db, $lngstr, $srv_settings;
        ...
        if ($G_SESSION['yt_state'] == TEST_STATE_QREVIEW) {
            ...
            $qry_str = "INSERT INTO " . $srv_settings['table_prefix'] . "results_answers (result_answerid,eventid, resultid, questionid, test_questionid, result_answer_text, result_answer_points, result_answer_iscorrect, result_answer_feedback, result_answer_timespent, result_answer_timeexceeded) VALUES (" . $i_questionno . ", " . $G_SESSION['eventid'] . ", " . $G_SESSION['resultid'] . ", " . $i_questionid . ", " . $i_questionno_real . ", " . $g_db->qstr($i_answer_text, get_magic_quotes_gpc()) . ", " . $i_answer_points . ", " . $i_answer_correct . ", '', " . $i_timespent . ", " . $i_timeexceeded . ")";
        }
        $g_db->Execute($qry_str);
    ...
    {
        return $this->truncateString(trim(strip_tags($this->convertTextAreaHTML(false, $i_html))), $i_length);
    ...
    function deleteQuestionLink($testid, $test_questionid,$srv_settings)
    {
        if ($this->m_bd->update("DELETE FROM " . $srv_settings['table_prefix'] . "tests_questions WHERE testid=" . $testid . " AND test_questionid=" . $test_questionid) === false)
            showDBError('deleteQuestionLink', 1);

        $i_rSet2 = $this->m_bd->Execute("SELECT test_questionid FROM " . $srv_settings['table_prefix'] . "tests_questions WHERE testid=" . $testid . " AND test_questionid>" . $test_questionid . " ORDER BY test_questionid");
        if ($i_rSet2) {
            $i_counter = $test_questionid - 1;
            foreach ($i_rSet2 as $key=>$value) {
                $i_counter++;
                if ($value["test_questionid"] != $i_counter)
                    $this->m_bd->update("UPDATE " . $srv_settings['table_prefix'] . "tests_questions SET test_questionid=" . $i_counter . " WHERE testid=" . $testid . " AND test_questionid=" . $value["test_questionid"]);
                //$i_rSet2->MoveNext();
            }
            //$i_rSet2->Close();
        }
    ...
    function createQuestionLink($testid, $questionid,$srv_settings)
    {
        $i_questioncount = 0;

        $i_questioncount = $this->getRecordCount($srv_settings['table_prefix'] . 'tests_questions', 'testid=' . $testid);
        $i_questioncount++;

        if ($this->m_bd->insert("INSERT INTO " . $srv_settings['table_prefix'] . "tests_questions (test_questionid, testid, questionid) VALUES(" . $i_questioncount . ", " . $testid . ", " . $questionid . ")") === false)
