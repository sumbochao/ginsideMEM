...
<script type="text/javascript">
    $(document).ready(function() {
        $('#onSubmitPopup').on('click',function(){
            ...
            $.ajax({
                type: "POST",
                dataType: 'jsonp',
                url: "<?php echo $url; ?>/cms/toolgettop/add_config",
                ...
                beforeSend: function(  ) {
                    ...
                    window.location.href='?control=toolgettop&func=edit&view=wheel&&ids=<?php echo $_GET['ids'];?>';
                    //enable button
        ...
        $('#onSubmitUpdate').on('click',function(){alert('345');
            ...
            $.ajax({
                type: "POST",
                dataType: 'jsonp',
                url: "<?php echo $url; ?>/cms/toolgettop/edit_config",
                data: $(".popupFrmSendChest").serializeArray(),
                beforeSend: function(  ) {
    ...
    function getData(){
        $.ajax({
                url: "<?php echo $url;?>/cms/toolgettop/index_config?iditems=<?php echo $_GET['ids']; ?>",
                dataType: 'jsonp',
                method: "POST",
                bRetrieve: true, 
                bDestroy: true,
                            ...
                            },                            
                           {
                            ...
                            },
							{
                             ...
                             { 
                                ...
                                mData: "id",
                                mRender: function(data){
                                    return "<a href='javascript:;' onclick='editConfig("+data+")'>Edit</a> <span style='padding-left:10px'><a href='?control=toolgettop&func=deleteconfig&view=wheel&ids="+data+"&wheel=<?php echo $_GET['ids'];?>'>Delete</a></span>";
                                }
                        ...
                        <div class="control-group frmedit totalchest">
                            <div class="group1">
								...
								<div class="form-group">
                                    ...
                                    <input id="rankto" name="rankto" type="text" value="" class="span3 validate[required]">
                                </div>
								...
								<div class="form-group" style="width:30%">
                                    <label class="control-label">Owner Guild:</label>
                                    <span class="loadStatus">
                                        Enable:<input type="radio" name="isower" class="isower" id="isower" value="1" style="display:inline; width: 10%;"/>
                                        &nbsp;&nbsp;
                                        Disable:<input type="radio" name="isower" class="isower" id="isower" value="0" checked="checked"  style="display:inline; width: 10%;"/>
                                    </span>
                                </div>
                                <div class="clear"></div>
                            </div>
...
<script>
    function editConfig(ids){
        $.ajax({
            url: "<?php echo $urlConfig;?>?control=toolgettop&func=edit_config&view=wheel",
            type:"GET",
            data:{ids:ids},
            dataType:"json",
            success:function(f){
