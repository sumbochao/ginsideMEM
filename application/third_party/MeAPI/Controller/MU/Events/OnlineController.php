<?php
class MeAPI_Controller_MU_Events_OnlineController implements MeAPI_Controller_MU_Events_OnlineInterface {
    protected $_response;
    private $CI;
    private $url_service;
    ...
    public function __construct() {
        ...
        if($_SERVER['SERVER_ADDR']=='127.0.0.1'){
            $this->url_service = 'http://localhost.service.mu.mobo.vn/mu/index.php';
        }else{
            $this->url_service = 'http://game.mobo.vn/mu';
        }
    ...
    public function listserver(MeAPI_RequestInterface $request){
        ...
        $this->data['s_id']= $s_id;
        echo $this->CI->load->view('game/mu/Events/online/filters/listserver', $this->data, true);
        die();
    }
    public function index(MeAPI_RequestInterface $request){
        ...
        $this->CI->template->write_view('content', 'game/mu/Events/online/'.$_GET['view'].'/index', $this->data);
    ...
    public function add(MeAPI_RequestInterface $request){
        ...
        switch ($_GET['view']){
            ...
            case 'filters':
                ...
                $linkinfoslbEvent = $this->url_service.'/cms/online_nhanqua/slbevent';
        ...
        $this->data['url_service'] = $this->url_service;
        $this->CI->template->write_view('content', 'game/mu/Events/online/'.$_GET['view'].'/add', $this->data);
        $this->_response = new MeAPI_Response_HTMLResponse($request, $this->CI->template->getHTML());
    }
    public function edit(MeAPI_RequestInterface $request){
        ...
        $linkinfo = $this->url_service.'/cms/online_nhanqua/get_'.$_GET['view'].'?id='.$id;
    ...
    public function delete(MeAPI_RequestInterface $request) {
        $this->authorize->validateAuthorizeRequest($request, 0); 
        $id = $_GET['id'];
        $linkInfo = $this->url_service.'/cms/online_nhanqua/delete_'.$_GET['view'].'?id='.$id;
    ...
    public function excel(MeAPI_RequestInterface $request){
        $this->authorize->validateAuthorizeRequest($request, 0);
        $linkInfo = $this->url_service.'/cms/online_nhanqua/excel?start='.strtotime($_GET['start']).'&end='.strtotime($_GET['end']);
        $j_items = file_get_contents($linkInfo);
        $listItem = json_decode($j_items,true);
        
        header('Content-type: application/excel');
        $filename = 'online_nhanqua.xls';
