...
<div id="content-t" class="content_form" style="min-height:0; padding-top:10px">
       ...
       <form id="appForm" action="<?php echo base_url();?>?control=<?php echo $_GET['control'];?>&func=<?php echo $_GET['func'];?>&id_categories=<?php echo $_GET['id_categories'];?>&id_game=<?php echo $_GET['id_game'];?>&type=filter" method="post" name="appForm" enctype="multipart/form-data">
       <div class="filter">
...
<table style="float:left;" border="0" class="table table-striped table-bordered table-condensed table-hover" id="tblsort">
            ...
            <tbody>
                ...
                <tr>
                	<td>
                    <a href="<?php echo base_url()."?control=".$_GET['control']."&func=edit&id=".$v['id']."&id_game=".intval($_GET['id_game'])."&id_categories=".$v['id_categories']; ?>" title="Sửa"><img border="0" title="Sửa" src="<?php echo base_url().'assets/img/icon_edit.png'; ?>"></a>
                        
<a onclick="checkdel('appForm','<?php echo base_url()."?control=".$_GET['control']."&func=delete&id=".$v['id']."&id_categories=".$v['id_categories']."&id_game=".intval($_GET['id_game']); ?>')" href="javascript:void(0);" title="Xóa"><img border="0" title="Xóa" src="<?php echo base_url().'assets/img/icon_del.png'; ?>"></a> 
                    </td>
                    <td style="background:#990"><strong style="color:#903;font-size:10px;"><?php echo $slbCategories[$v['id_categories']]['names'];?></strong></td>
                    <td><i style="font-size:12px;font-weight: bold;"><a href="<?php echo base_url()."?control=".$_GET['control']."&func=edit&id=".$v['id']."&id_game=".intval($_GET['id_game'])."&id_categories=".$v['id_categories']; ?>" title="<?php echo $v['admin_request'];?>"><?php echo $v['titles'];?></a></i></td>
                    <td><?php echo $cate->CreateControlGroup(intval($_GET['id_game']),$v['id'],0); ?></td>
...
function calljavascript(i,id){
		switch(i){
			 ...
			 break;
			 case 1: // add new
			 		window.location.href='<?php $arr=array_merge($_POST,$_GET);echo base_url()."?control=".$_GET['control']."&func=add&id_categories=".$arr['id_categories']."&id_game=".intval($arr['id_game']) ?>';
			 break;
			 case 2: //tim kiem
