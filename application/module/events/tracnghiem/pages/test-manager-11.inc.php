...
while(strlen($f_import_document) > 0 || $i_nCurrNearest >= 0) {  
 ...
 case 1: $f_import_document = substr($f_import_document, strlen($f_strQSep), strlen($f_import_document)-strlen($f_strQSep)); break; 
 case 2: $f_import_document = substr($f_import_document, strlen($f_strPreQSep), strlen($f_import_document)-strlen($f_strPreQSep)); break; 
 case 3: $f_import_document = substr($f_import_document, strlen($f_strExplainQSep), strlen($f_import_document)-strlen($f_strExplainQSep)); break; 
 case 4: $f_import_document = substr($f_import_document, strlen($f_strA1Sep), strlen($f_import_document)-strlen($f_strA1Sep)); break; 
 case 8: $f_import_document = substr($f_import_document, strlen($f_strA2Sep), strlen($f_import_document)-strlen($f_strA2Sep)); break; 
 case 5: $f_import_document = substr($f_import_document, strlen($f_strCorrectASep), strlen($f_import_document)-strlen($f_strCorrectASep)); break; 
 case 6: $f_import_document = substr($f_import_document, strlen($f_strPointsSep), strlen($f_import_document)-strlen($f_strPointsSep)); break; 
 case 7: $f_import_document = substr($f_import_document, strlen($f_strQTypeSep), strlen($f_import_document)-strlen($f_strQTypeSep)); break; 
 case 9: $f_import_document = substr($f_import_document, strlen($f_strSectionSep), strlen($f_import_document)-strlen($f_strSectionSep)); break; 
	} 
...
case 7: 
 $i_strData = str_replace("\n", '', $i_strData);
$i_strData = str_replace("\r", '', $i_strData);
$i_strData = strtolower(trim($i_strData));
$i_nData = (int)$i_strData;
if($i_nData >= 1) {
 $i_strData = $i_nData;
}
switch($i_strData) {
 case 2:
 case $lngstr['page_testmanager_import']['question_type_multiple_answer']:
 $f_question_type = QUESTION_TYPE_MULTIPLEANSWER;
break;
case 5:
 case $lngstr['page_testmanager_import']['question_type_short_answer']:
 $f_question_type = QUESTION_TYPE_FILLINTHEBLANK;
break;
case 7:
 case $lngstr['page_testmanager_import']['question_type_essay']:
 $f_question_type = QUESTION_TYPE_ESSAY;
break;
case 1:
 case $lngstr['page_testmanager_import']['question_type_multiple_choice']: 
...
$i_nSectionSep = strpos($f_import_document, $f_strSectionSep);  
	$i_nNearestTmp = -1;
$i_nNearestPosTmp = IGT_MAX_UNSIGNED_INT;
