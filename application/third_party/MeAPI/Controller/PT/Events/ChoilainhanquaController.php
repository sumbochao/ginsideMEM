<?php
class MeAPI_Controller_PT_Events_ChoilainhanquaController implements MeAPI_Controller_PT_Events_ChoilainhanquaInterface {
    protected $_response;
    private $CI;
    private $url_service;
    ...
    public function index(MeAPI_RequestInterface $request){
        ...
        $this->CI->template->write_view('content', 'game/pt/Events/choilainhanqua/'.$_GET['view'].'/index', $this->data);
    ...
    public function add(MeAPI_RequestInterface $request){
        ...
        $this->data['url_service'] = $this->url_service;
        $this->CI->template->write_view('content', 'game/pt/Events/choilainhanqua/'.$_GET['view'].'/add', $this->data);
        $this->_response = new MeAPI_Response_HTMLResponse($request, $this->CI->template->getHTML());
    }
    public function edit(MeAPI_RequestInterface $request){
        ...
        $linkinfo = $this->url_service.'/cms/choilainhanqua/gettem_'.$_GET['view'].'?id='.$id;
    ...
    public function history(MeAPI_RequestInterface $request){
        ...
        $this->data['url_service'] = $this->url_service;
        $this->CI->template->write_view('content', 'game/pt/Events/choilainhanqua/history', $this->data);
        $this->_response = new MeAPI_Response_HTMLResponse($request, $this->CI->template->getHTML());
    }
    public function excel(MeAPI_RequestInterface $request){
        $this->authorize->validateAuthorizeRequest($request, 0);
        $linkInfo = $this->url_service.'/cms/choilainhanqua/excel?&start='.strtotime($_GET['start']).'&end='.strtotime($_GET['end']);
        $j_items = file_get_contents($linkInfo);
