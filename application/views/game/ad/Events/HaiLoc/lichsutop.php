    ...
    <script type="text/javascript">
        $(function () {
            $("#tournament_id").change(function () {
                //getData($(this).val());
                window.location.href = "/?control=hailoc_ad&func=lichsutop&tournament_id=" + $(this).val() + "&startdate=" + $("#startdate").val() + "&enddate=" + $("#enddate").val() + "#lichsutop";
            });
        });

        $(document).ready(function () { 
            $('#create_boxitem').on('click', function () {
                window.location.href = "/?control=hailoc_ad&func=lichsutop&tournament_id=" + $("#tournament_id").val() + "#lichsutop";
            });

            $('#export_excel').on('click', function () {
                window.location.href = "http://game.mobo.vn/acdau/cms/toolhailoc/get_exchange_history_top_excel?tournament_id=" + $("#tournament_id").val();
            });
        ...
        function getData(tournament_id) {
            $("#loading_img").show();
            $.ajax({
                url: "http://game.mobo.vn/acdau/cms/toolhailoc/get_exchange_history_top?tournament_id=" + tournament_id + "&startdate=" + $("#startdate").val() + "&enddate=" + $("#enddate").val() + "&pagesize=10000",
                dataType: 'jsonp',
                method: "POST",
                bRetrieve: true,
                bDestroy: true,
