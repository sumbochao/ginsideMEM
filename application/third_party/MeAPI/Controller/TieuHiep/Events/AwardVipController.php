<?php
class MeAPI_Controller_TieuHiep_Events_AwardVipController implements MeAPI_Controller_TieuHiep_Events_AwardVipInterface {
    protected $_response;
    private $CI;
    private $url_service;
    ...
    public function __construct() {       
        ...
        if($_SERVER['SERVER_ADDR']=='127.0.0.1'){
            $this->url_service = 'http://localhost.game.mobo.vn/tieuhiep/index.php';
        }else{
            $this->url_service = 'http://game.mobo.vn/tieuhiep';
        }
    ...
    public function index(MeAPI_RequestInterface $request){
        ...
        switch ($_GET['view']){
            case 'event':
                ...
                $this->CI->template->write_view('content', 'game/tieuhiep/Events/awardvip/event/index', $this->data);
                break;
            case 'config':
                $this->data['title'] = 'QUẢN LÝ CẤU HÌNH';
                $this->CI->template->write_view('content', 'game/tieuhiep/Events/awardvip/config/index', $this->data);
                break;
            case 'item':
                $this->data['title'] = 'QUẢN LÝ ITEM';
                $this->CI->template->write_view('content', 'game/tieuhiep/Events/awardvip/item/index', $this->data);
                break;
        } 
    ...
    public function add(MeAPI_RequestInterface $request) {
        ...
        switch ($_GET['view']){
            case 'event':
                ...
                $this->CI->template->write_view('content', 'game/tieuhiep/Events/awardvip/event/add', $this->data);
                break;
            case 'config':
                ...
                $this->CI->template->write_view('content', 'game/tieuhiep/Events/awardvip/config/add', $this->data);
                break;
            case 'item':
                $this->data['title'] = 'THÊM ITEM';
                $this->CI->template->write_view('content', 'game/tieuhiep/Events/awardvip/item/add', $this->data);
                break;
        }
        $this->_response = new MeAPI_Response_HTMLResponse($request, $this->CI->template->getHTML());
    }
    ...
    public function delete(MeAPI_RequestInterface $request) {
        ...
        switch ($_GET['view']){
            case 'event':
                ...
                $j_items = file_get_contents($linkInfo);
                redirect(base_url().'?control='.$_GET['control'].'&func=index&view=event&module=all#event');
                break;
            case 'config':
                $id = $_GET['id'];
                $linkInfo = $this->url_service.'/cms/awardvip/delete_config?id='.$id;
                $j_items = file_get_contents($linkInfo);
                redirect(base_url().'?control='.$_GET['control'].'&func=index&view=config&module=all#config');
                break;
            case 'item':
                $id = $_GET['id'];
                $linkInfo = $this->url_service.'/cms/awardvip/delete_item?id='.$id;
                $j_items = file_get_contents($linkInfo);
                redirect(base_url().'?control='.$_GET['control'].'&func=index&view=item&module=all#item');
                break;
        }
    }
    public function history(MeAPI_RequestInterface $request) {
        ...
        $this->authorize->validateAuthorizeRequest($request, 0);        
        $this->CI->template->write_view('content', 'game/tieuhiep/Events/awardvip/history', $this->data);
        $this->_response = new MeAPI_Response_HTMLResponse($request, $this->CI->template->getHTML());
    }
        ...
        if(count($listItem)){
            foreach($listItem as $v){
                ...
                $data .= '<tr>
                    ...
                    <td align="center">VIP từ '.$condition['from'].' đến '.$condition['to'].' giá '.$condition['gold'].' gold</td>
