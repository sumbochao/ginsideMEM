    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            $('#create_boxitem').on('click', function () {
                window.location.href = "/?control=managercontributor&func=historyuser&startdate=" + $("#startdate").val() + "&enddate=" + $("#enddate").val() + "#lichsu";
            });
...
//                url: "http://mem.net.vn/cms/toolsalarycollaborator/games_list",
        ...
        function getData(gamelistId) {
            $("#loading_img").show();
            $.ajax({
                url: "http://mem.net.vn/cms/toolsalarycollaborator/get_log_collaborator_approval_history?startdate=" + $("#startdate").val() + "&enddate=" + $("#enddate").val() + "&gamelistId=" + gamelistId + "&pagesize=10000",
                dataType: 'jsonp',
                method: "POST",
                bRetrieve: true,
                bDestroy: true,
                ...
                success: function (data, textStatus, jqXHR) {
                    $('#data_table').dataTable({
                        ...
                        aoColumns: [
                            ...
                            {
                                sTitle: "Game tham gia",
                                mData: "game_ids",
                                mRender: function (data) {
                                    obj2 = jQuery.parseJSON(data);
                                    var html = "";
                                    $.each(obj2, function (key2, value2) {
                                        html = html + "Game : " + value2["name"] + " - Quota : " + value2["quota"] + "</br>";
                                    });
                                    return html;
                                }
                            },
                            {
                                ...
                                mRender: function (data) {
                                    if (data == 0) {
                                        ...
                                        return "<span style='color:green'>Duyệt</span>";
                                    } else {
