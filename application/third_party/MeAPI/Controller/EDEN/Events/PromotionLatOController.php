...
class MeAPI_Controller_EDEN_Events_PromotionLatOController implements MeAPI_Controller_EDEN_Events_PromotionLatOInterface {
    ...
    public function productbox(MeAPI_RequestInterface $request) {
        $this->authorize->validateAuthorizeRequest($request, 0);
        $this->CI->template->write_view('content', 'game/eden/Events/LatO/productbox', $this->data);
        $this->_response = new MeAPI_Response_HTMLResponse($request, $this->CI->template->getHTML());
    }
    ...
    public function productboxediting(MeAPI_RequestInterface $request) {
        ...
        //load info editchest
        $ids = $_GET['ids'];
        $linkinfo = 'http://service.eden.mobo.vn/cms/promotion_lato/boxitemdetailload?ids='.$ids;
        $infoDetail = file_get_contents($linkinfo);
        $infodetail = json_decode($infoDetail,true);

        $this->data['infodetail'] = $infodetail;      
        
        $this->CI->template->write_view('content', 'game/eden/Events/LatO/productboxediting', $this->data);
        $this->_response = new MeAPI_Response_HTMLResponse($request, $this->CI->template->getHTML());
    }
    
    public function thongke(MeAPI_RequestInterface $request) {
        $this->authorize->validateAuthorizeRequest($request, 0);
        $this->CI->template->write_view('content', 'game/eden/Events/LatO/thongke', $this->data);
        $this->_response = new MeAPI_Response_HTMLResponse($request, $this->CI->template->getHTML());
    }
    
    public function logdoiqua(MeAPI_RequestInterface $request) {
        $this->authorize->validateAuthorizeRequest($request, 0);
        $this->CI->template->write_view('content', 'game/eden/Events/LatO/tralogdoiqua', $this->data);
        $this->_response = new MeAPI_Response_HTMLResponse($request, $this->CI->template->getHTML());
    }
    
    public function logthamgia(MeAPI_RequestInterface $request) {
        $this->authorize->validateAuthorizeRequest($request, 0);
        $this->CI->template->write_view('content', 'game/eden/Events/LatO/logthamgia', $this->data);
        $this->_response = new MeAPI_Response_HTMLResponse($request, $this->CI->template->getHTML());
    }
    
    public function logdoiluotbangtien(MeAPI_RequestInterface $request) {
        $this->authorize->validateAuthorizeRequest($request, 0);
        $this->CI->template->write_view('content', 'game/eden/Events/LatO/logdoiluotbangtien', $this->data);
        $this->_response = new MeAPI_Response_HTMLResponse($request, $this->CI->template->getHTML());
    }
    
    public function clearcached(MeAPI_RequestInterface $request) {
        $this->authorize->validateAuthorizeRequest($request, 0);
        $this->CI->template->write_view('content', 'game/eden/Events/LatO/clearcached', $this->data);
        $this->_response = new MeAPI_Response_HTMLResponse($request, $this->CI->template->getHTML());
    }
