<?php
class MeAPI_Controller_FISH_Events_BuygemController implements MeAPI_Controller_FISH_Events_BuygemInterface {
    protected $_response;
    private $CI;
    public $url;
	...
	public function index(MeAPI_RequestInterface $request){
        ...
        switch ($_GET['view']){
            case 'config':
                ...
                $this->CI->template->write_view('content', 'game/fish/Events/toolbuygem/config/index', $this->data);
                break;
            case 'item':
    ...
    public function config(MeAPI_RequestInterface $request) {
        ...
        $this->CI->template->write_view('content', 'game/fish/Events/toolbuygem/config', $this->data);
        $this->_response = new MeAPI_Response_HTMLResponse($request, $this->CI->template->getHTML());
    }
    public function history(MeAPI_RequestInterface $request) {
        $this->authorize->validateAuthorizeRequest($request, 0);        
        $this->CI->template->write_view('content', 'game/fish/Events/toolbuygem/history', $this->data);
        $this->_response = new MeAPI_Response_HTMLResponse($request, $this->CI->template->getHTML());
    }
	public function historygroup(MeAPI_RequestInterface $request) {
        $this->authorize->validateAuthorizeRequest($request, 0);        
        $this->CI->template->write_view('content', 'game/fish/Events/toolbuygem/historygroup', $this->data);
        $this->_response = new MeAPI_Response_HTMLResponse($request, $this->CI->template->getHTML());
    }
	...
	public function add(MeAPI_RequestInterface $request) {
        ...
        switch ($_GET['view']){
            case 'config':
                ...
                $this->CI->template->write_view('content', 'game/fish/Events/toolbuygem/config/add', $this->data);
                break;
            case 'item':
                $this->data['title'] = 'THÊM ITEM';
                $this->CI->template->write_view('content', 'game/fish/Events/toolbuygem/item/add', $this->data);
                break;
        }
        $this->_response = new MeAPI_Response_HTMLResponse($request, $this->CI->template->getHTML());
    }
    public function edit(MeAPI_RequestInterface $request) {
        ...
        switch ($_GET['view']){
            case 'config':
                ...
                $id = $_GET['ids'];
                $linkInfo = $this->url.'/event/buygem/getconfigId?idx='.$id."&type=config";
            ...
            case 'item':
                ...
                $id = $_GET['ids'];
                $linkInfo = $this->url.'/event/buygem/getconfigId?idx='.$id."&type=item";
                $j_items = file_get_contents($linkInfo);
                $this->data['items'] = json_decode($j_items,true);
                $this->CI->template->write_view('content', 'game/fish/Events/toolbuygem/item/add', $this->data);
                break;
