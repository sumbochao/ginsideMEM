...
class MeAPI_Controller_PT_Events_LevelController implements MeAPI_Controller_PT_Events_LevelInterface {
    ...
    public function index(MeAPI_RequestInterface $request) {
        ...
        $this->CI->template->write_view('content', 'game/pt/Events/qualv/'.$layout.'/index', $this->data);
    ...
    public function add(MeAPI_RequestInterface $request) {
        ...
        $this->CI->template->write_view('content', 'game/pt/Events/qualv/'.$layout.'/add', $this->data);
        $this->_response = new MeAPI_Response_HTMLResponse($request, $this->CI->template->getHTML());
    }
	function delete(MeAPI_RequestInterface $request){
        ...
        $linkinfo = $this->url_link.'/cms/qualv/delete_'.$layout.'?ids='.$ids;
        $infoDetail = file_get_contents($linkinfo);
        $datainfojson = json_decode($infoDetail,true);
        header('location: /?control=qualv&func=index&view='.$layout);
    }
	function deleteconfig(MeAPI_RequestInterface $request){
		$this->authorize->validateAuthorizeRequest($request, 0);
        $ids = $request->input_get('ids');
        $linkinfo = $this->url_link.'/cms/qualv/delete_config?ids='.$ids;
		
        $infoDetail = file_get_contents($linkinfo);
        $datainfojson = json_decode($infoDetail,true);
        header('location: /?control=qualv&func=edit&view=wheel&ids='.$request->input_get('wheel') );
    }
    ...
    public function edit(MeAPI_RequestInterface $request) {
        ...
        $ids =$request->input_get('ids');
        $linkinfo = $this->url_link.'/cms/qualv/getitem_'.$layout.'?ids='.$ids;
        $infoDetail = file_get_contents($linkinfo);
        $datainfojson = json_decode($infoDetail,true);

        $linkinfo = $this->url_link.'/cms/qualv/index_item';
    ...
    public function clearcached(MeAPI_RequestInterface $request) {
        $this->authorize->validateAuthorizeRequest($request, 0);
        $this->CI->template->write_view('content', 'game/pt/Events/qualv/clearcached', $this->data);
        $this->_response = new MeAPI_Response_HTMLResponse($request, $this->CI->template->getHTML());
    }    
	...
	public function history(MeAPI_RequestInterface $request){
        $this->authorize->validateAuthorizeRequest($request, 0);
        $this->CI->template->write_view('content', 'game/pt/Events/qualv/history', $this->data);
        $this->_response = new MeAPI_Response_HTMLResponse($request, $this->CI->template->getHTML());
    }
    public function excel(MeAPI_RequestInterface $request){
        $this->authorize->validateAuthorizeRequest($request, 0);
        $linkInfo = $this->url_link.'/cms/qualv/excel?start='.strtotime($_GET['start']).'&end='.strtotime($_GET['end']);
