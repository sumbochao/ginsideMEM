...
class MeAPI_Controller_MestoreVersionController {
    ...
    public function index(MeAPI_RequestInterface $request) {
        ...
        if (isset($_GET['type']) && $_GET['type'] == "filter") {
            ...
            $arrGame = explode('-', $arrParam['cbo_app']);
            $arrParam['cbo_app'] = $arrGame[0];
			...
			if($arrGame[1]=='global'){
				...
				$this->data['views'] = 'global';
				$this->data['api_view_gsv'] = $reponse;
			}else{	
				$reponse = $this->CI->Graph_Inside_API->view_msv($url_param);
				$this->data['api_view_msv'] = $reponse;
			}
    ...
    public function add(MeAPI_RequestInterface $request) {
        ...
        if ($this->CI->input->post()) {
            // lay so OTP
            $serviceid = explode('-', $this->CI->input->post('cbo_app'));
            if ($serviceid[0] == "124") {
                $otp = $this->CI->Graph_Inside_API->get_otp_thai();
                $param = array(
                    "control" => "inside",
                ...
                $otp = $this->CI->Graph_Inside_API->get_otp();
                if($serviceid[1] == "global"){
                    $param = array(
                        "control" => "inside",
                        "func" => "gsv_add",
                        "gsv_id" => $this->CI->input->post('txt_msv'),
                        "status" => $this->CI->input->post('cbo_status'),
                        "platform" => $this->CI->input->post('cbo_platform'),
                        "service_id" => $serviceid[0],
                        "app" => "ginside",
            ...
            if($serviceid[1] == "global"){
                $param_c = array(
                    "msv_id" => "gsv_" . $this->CI->input->post('txt_msv'),
                    "status" => $this->CI->input->post('cbo_status'),
                    "platform" => $this->CI->input->post('cbo_platform'),
                    "service_id" => $serviceid[0],
            ...
            if ($serviceid[0] == "124") {
				...
				if($serviceid[1] == "global"){
					$kq = $this->CI->Graph_Inside_API->gsv_add($param);
				}else{
    ...
    public function getcert() {
		$id_game = explode('-',$_GET['id_game']);
		$_GET['id_game'] = $id_game[0];
