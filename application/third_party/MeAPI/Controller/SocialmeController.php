...
class MeAPI_Controller_SocialmeController implements MeAPI_Controller_SocialmeInterface {
    ...
    public function loadtypeeventcontent(MeAPI_RequestInterface $request){
        ...
        foreach($getcategoryall as $val){
            $edit = "<a href='".base_url()."?control=socialme&func=addmenu&games=".$idgame."&id=".$val['service_id']."'><img src='".base_url()."assets/img/icon/icon_edit.png'></a>";
            $tbody.= "<tr>";
            $tbody.="<td>".$i."</td>";
    ...
    public function addmenu(MeAPI_RequestInterface $request){
        ...
        $loadlanguage = $this->CI->SocialmeModel->getalllanguage();
        $parseArraylang = array();
        foreach($loadlanguage as $val){
            $parseArraylang[$val['alias']] = $val['name'];
        }

        $this->data['language'] = $parseArraylang;
		if( (isset($_GET['games']) && $_GET['games']>=1) || $_POST['id_app'] ){
            foreach($loadnavigators as $val){
                if( ($_GET['games'] == $val['id'] )  || ($_POST['id_app'] == $val['id'] ) ){
                    $this->data['slbApp'][] = $val;
                    $linkdomain = $val['domain'];
                    $this->data['domain'] = $linkdomain;
                    break;
                }
            }
            if(empty($linkdomain) ){
        ...
        $this->data['itemslist'] = $getcategorydetail;
        //echo $linkdomain.$this->configlink['update'];
        if ($this->CI->input->post() && count($this->CI->input->post())>=1) {
            ...
            $jsonitem = array();
            if(isset($_POST['server_id']) && !empty($_POST['server_id']) && is_array($_POST['server_id'])){
                $start_service = $_POST['service_start'];
                $end_service = $_POST['service_end'];
                foreach($_POST['server_id'] as $key=>$val){
                    $jsonitem[$val] = array('server_id'=>$val,'service_start'=>$start_service[$key],'service_end'=>$end_service[$key]);
                }
            }else{
				$service_start = $_POST['startdate'];
				$service_end = $_POST['enddate'];
			}
            ...
            if(!empty($service_url)  && !empty($service_title)   ){
                ...
                $arrayevent =array();
                $paramsinsert= array(
                    "service_trustip"=>trim($service_trustip),
                    ...
                    "service_language"=>json_encode($parseArraylang),
				...
				if(empty($paramsinsert['service_end'])){
					unset($paramsinsert['service_end']);
				}
				if(empty($paramsinsert['service_start'])){
					unset($paramsinsert['service_start']);
				}

                if(!empty($id_sevice) && $id_sevice >=1){//echo "<pre>";print_r($paramsinsert);die();
    ...
    public function configlanguage(MeAPI_RequestInterface $request){
        $this->CI->load->MeAPI_Model('SocialmeModel');
        $loadalllanguage = $this->CI->SocialmeModel->getalllanguage();
        $this->data['listconfig'] = $loadalllanguage;
        $this->CI->template->write_view('content', 'socialme/toolconfig_language_index', $this->data);
        $this->_response = new MeAPI_Response_HTMLResponse($request, $this->CI->template->getHTML());
    }
    public function language(MeAPI_RequestInterface $request){
        $this->authorize->validateAuthorizeRequest($request, 0);
        $this->CI->load->MeAPI_Model('SocialmeModel');
        ...
        if(isset($_GET['id']) && is_numeric($_GET['id'])){
            $idget = $_GET['id'];
            $getconfig = $this->CI->SocialmeModel->getlanguage($idget);
        }
        ...
        if ($this->CI->input->post() && count($this->CI->input->post())>=1) {
            ...
            if(!empty($id) && $id >=1){
                ...
                $statusupdate = $this->CI->SocialmeModel->update($this->tbl_language,$paramsinsert,$whereUpdate);
                if($statusupdate)
                    $this->data['error']='Cập nhât thành công';
                else
                    $this->data['error']='Cập nhât thất bại';
                ...
                //kiem tra da ton tai id_app va note nay` chua...neu ton tai thi hok cho insert
                $statusinsert = $this->CI->SocialmeModel->insert_id($this->tbl_language,$paramsinsert);
                if($statusinsert != false)
                    $this->data['error']='Thêm thành công';
                else
                    $this->data['error']='Thêm thất bại';
        ...
        $this->CI->template->write_view('content', 'socialme/toolconfig_language_add', $this->data);
        $this->_response = new MeAPI_Response_HTMLResponse($request, $this->CI->template->getHTML());
    }
