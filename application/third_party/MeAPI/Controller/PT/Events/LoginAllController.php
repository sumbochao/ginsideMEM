...
class MeAPI_Controller_PT_Events_LoginAllController implements MeAPI_Controller_PT_Events_LoginAllInterface {
    ...
    private $url_process = "http://game.mobo.vn/phongthan/cms/qualoginall/";
    //private $url_link = "http://game.mobo.vn/bog";
	private $url_link = "http://game.mobo.vn/phongthan/cms/qualoginall/";
	
	private $goi = array(1=>"Gói 1",2=>"Gói 2",3=>"Gói 3");
    ...
    public function index(MeAPI_RequestInterface $request) {
        ...
        $this->CI->template->write_view('content', 'game/pt/Events/qualoginall/'.$layout.'/index', $this->data);
    ...
    public function add(MeAPI_RequestInterface $request) {
        ...
        $this->CI->template->write_view('content', 'game/pt/Events/qualoginall/'.$layout.'/add', $this->data);
        $this->_response = new MeAPI_Response_HTMLResponse($request, $this->CI->template->getHTML());
    }
	function delete(MeAPI_RequestInterface $request){
        ...
        $linkinfo = $this->url_link.'delete_'.$layout.'?ids='.$ids;
        $infoDetail = file_get_contents($linkinfo);
        $datainfojson = json_decode($infoDetail,true);
        header('location: /?control=qualoginall&func=index&view='.$layout);
    }
	function deleteconfig(MeAPI_RequestInterface $request){
		$this->authorize->validateAuthorizeRequest($request, 0);
        $ids = $request->input_get('ids');
        $linkinfo = $this->url_link.'delete_config?ids='.$ids;
		
        $infoDetail = file_get_contents($linkinfo);
        $datainfojson = json_decode($infoDetail,true);
        header('location: /?control=qualoginall&func=edit&view=wheel&ids='.$request->input_get('wheel') );
    }
    ...
    public function edit(MeAPI_RequestInterface $request) {
        ...
        $ids =$request->input_get('ids');
        $linkinfo = $this->url_link.'getitem_'.$layout.'?ids='.$ids;
		$infoDetail = file_get_contents($linkinfo);
        $datainfojson = json_decode($infoDetail,true);

        $linkinfo = $this->url_link.'/index_item';
    ...
    public function clearcached(MeAPI_RequestInterface $request) {
        $this->authorize->validateAuthorizeRequest($request, 0);
        $this->CI->template->write_view('content', 'game/pt/Events/qualoginall/clearcached', $this->data);
        $this->_response = new MeAPI_Response_HTMLResponse($request, $this->CI->template->getHTML());
    }    
