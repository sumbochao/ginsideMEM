<?php
class MeAPI_Controller_KOA_Events_GiftcodemoboController implements MeAPI_Controller_KOA_Events_GiftcodemoboInterface {
    protected $_response;
    private $CI;
    private $url_service;
	private $url_process = "http://game.mobo.vn/lucgioi/cms/giftcodemobo/";
    ...
    public function __construct() {
        ...
        if($_SERVER['SERVER_ADDR']=='127.0.0.1'){
            $this->url_service = 'http://localhost.service.lucgioi.mobo.vn/bog/index.php';
        }else{
    ...
    public function index(MeAPI_RequestInterface $request){
        ...
        $this->CI->template->write_view('content', 'game/koa/Events/giftcodemobo/index', $this->data);
        $this->_response = new MeAPI_Response_HTMLResponse($request, $this->CI->template->getHTML());
    }
    public function add(MeAPI_RequestInterface $request){
        $this->data['title']= 'THÊM CẤU HÌNH';
        $this->authorize->validateAuthorizeRequest($request, 0);     
        $this->CI->template->write_view('content', 'game/koa/Events/giftcodemobo/add', $this->data);
        $this->_response = new MeAPI_Response_HTMLResponse($request, $this->CI->template->getHTML());
    }
	...
	public function history(MeAPI_RequestInterface $request) {
        ...
        $this->authorize->validateAuthorizeRequest($request, 0);     
        $this->CI->template->write_view('content', 'game/koa/Events/giftcodemobo/history', $this->data);
        $this->_response = new MeAPI_Response_HTMLResponse($request, $this->CI->template->getHTML());     
    }
