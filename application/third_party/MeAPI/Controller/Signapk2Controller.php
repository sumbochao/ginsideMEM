<?php
class MeAPI_Controller_Signapk2Controller{
    protected $_response;
    private $CI;
    private $_mainAction;
	private $limit;
	...
	//ftp info
	protected $ftp_server="115.78.161.88"; //10.8.36.180 ,115.78.161.124/134/88 , port web : 31112
	protected $ftp_server2="115.78.161.124";
	
	protected $web_server="115.78.161.88";
	protected $web_server2="115.78.161.124";
	
	protected $ftp_user_name="deploy";
	protected $ftp_user_pass="Deploy123@@@";
	
	protected $ftp_port="31111";
	protected $web_port="31112";
	...
	public function checknetwork(){
		...
		$net_def=$this->CI->Network->statusnetwork($this->web_server,$this->web_port);
		...
		if($net_def){
			...
			if($net_sec){
				$this->ftp_server=$this->ftp_server2;
				$this->web_server=$this->web_server2;
				$f = array(
                'error'=>'0',
	...
	public function add(MeAPI_RequestInterface $request){
		...
		$this->data['partner'] = $this->CI->ConfigAndroidModel->listItem(NULL);
        ...
        if ($this->CI->input->post()){
            ...
            if($this->CI->SignApkHistoryAppValidate->isVaild()){
				...
				$remote_file = "/signapk/";// files at ftp path
				 // set up basic connection  
				 //echo MeAPI_Controller_SignapkController::$ftp_server."-".$this->ftp_port;
				 $conn_id = ftp_connect($this->ftp_server,$this->ftp_port) or die ("Cannot connect to host");
				 ...
				 ftp_close($conn_id);
				 //get keystore
				 $configkey=explode("|",trim($_POST["cbo_partner"]));
				$post_array = array(
					"file_apk"=>trim($getItem['filenames_notsign']),
					"c"=>trim($getItem["channels"]),
					"package_name"=>trim($_POST["cbo_bunlderid"]),
					"keystore"=>trim($configkey[1]),
					"storepass"=>trim($configkey[2]),
					"alias"=>trim($configkey[3])
				);
                ...
                if(!empty($getItem['filenames_notsign'])){
                    ...
                    curl_setopt($ch, CURLOPT_URL, 'http://'.$this->web_server.':'.$this->web_port.'/signapk/signapk.php' );

                    curl_setopt($ch, CURLOPT_POSTFIELDS, $post_array);
                    $reponse = curl_exec($ch);
                    $arrReponse = json_decode($reponse,true);
                    curl_close($ch);
					//kiểm tra packaename có giống nhau hay không
					if(trim($arrReponse['package_name']) != trim($_POST["cbo_bunlderid"])){
						MeAPI_Log::writeCsv(array(date('Y-m-d H:i:s'),"Error Sign","user_id:".$_SESSION['account']['id']), "{$params['control']}_{$params['func']}_" . date('H'));
						redirect($this->_mainAction.'&type=unsuccess&pac_file='.$arrReponse['package_name'].'&pac_opt='.$_POST["cbo_bunlderid"]);
					}
                    if($arrReponse['error']==0){
						#15/04/2016
						$status_debug=$arrReponse['status_debug']=="debug values : 1"?"File APK đang ở chế độ debug.":"";
                        $arrParam = array(
						...
						if($result==1){
							redirect($this->_mainAction.'&type=success&id='.$getItem['id'].'&mess='.$status_debug);
						}
                    }else{
                         $arrParam = array(
                            'filenames_signed'=>$arrReponse['filenames_signed'],
	...
	public function showmsv(){
        if(isset($_GET['id_game'])){
            $data['slbMsv'] = $this->CI->SignapkModel->listMsvPlus($_GET['id_game'],$_GET['type_app'],$_GET['packagename'],$_GET['cert_id']);
	...
	public function getpackagename(){
        if(isset($_GET['id_game'])){
            ...
            $f = array(
                'error'=>'0',
                'messg'=>'Thành công',
                'html'=>$this->CI->load->view('signapk/cbo_bunlderid', $data, true)
            );
        }else{
            $data['list'] = array();
            $f = array(
