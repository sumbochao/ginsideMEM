...
class CertificateModel extends CI_Model {
    private $_db_slave;
    private $_table='tbl_sign_config_cert';
    ...
    public function listItem($arrParam=NULL,$options=null){  
        ...
        if(!empty($arrParam['keyword'])){
            $this->_db_slave->where("(`cert_name` LIKE '%{$arrParam['keyword']}%')", '', false); 
        }
        if(!empty($arrParam['idpartner'])){
            $this->_db_slave->where("idpartner",$arrParam['idpartner']); 
        }
