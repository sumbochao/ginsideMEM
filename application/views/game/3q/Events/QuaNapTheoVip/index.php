    ...
    <script type="text/javascript">
        $(function () {
            ...
            $('#create_tournament').on('click', function () {
                window.location.href = '/?control=quanaptheovip_3q&func=themgiaidau&module=all#giaidau';
            });
            ...
            $('#onSubmit').on('click', function () {
                ...
                $.ajax({
                    type: "POST",
                    dataType: 'jsonp',
                    url: "http://game.mobo.vn/3q/cms/toolquanaptheovip/edit_tournament_details",
        ...
        $(document).ready(function () {
            ...
            $("#chargingdatestart").jqxDateTimeInput({width: '150px', height: '28px', formatString: 'yyyy-MM-dd HH:mm:ss'});
            $("#chargingdateend").jqxDateTimeInput({width: '150px', height: '28px', formatString: 'yyyy-MM-dd HH:mm:ss'});
        ...
        function load_tournament_detail(id) {
            $.ajax({
                method: "GET",
                url: "http://game.mobo.vn/3q/cms/toolquanaptheovip/tournament_get_by_id?id=" + id,
                contentType: 'application/json; charset=utf-8',
                success: function (data) {
                    $("#id").val(data[0]["id"]);
                    ...
                    var date_calcula_gold = null;
                    if (data[0]["date_calcula_gold"] != "" && data[0]["date_calcula_gold"] != null) {
                        date_calcula_gold = new Date(data[0]["date_calcula_gold"]);
                    }
                    var date_calcula_gold_end = null;
                    if (data[0]["date_calcula_gold_end"] != "" && data[0]["date_calcula_gold_end"] != null) {
                        date_calcula_gold_end = new Date(data[0]["date_calcula_gold_end"]);
                    }

                    $("#startdate").jqxDateTimeInput('setDate', tournament_date_start);
                    $("#enddate").jqxDateTimeInput('setDate', tournament_date_end);
                    $("#chargingdatestart").jqxDateTimeInput('setDate', date_calcula_gold);
                    $("#chargingdateend").jqxDateTimeInput('setDate', date_calcula_gold_end);
                    $("#server_list").val(data[0]["tournament_server_list"]);
                    $("#charging_start").val(data[0]["charging_start"]);
                    $("#charging_min").val(data[0]["charging_min"]);
                    ...
                    <form id="frmSendChest" name="frmSendChest" action="" method="POST" onsubmit="return false;" autocomplete="off">
                        <div class="control-group">
                            ...
                            <div class="controls">
                                ...
                                <button type="button" id="create_tournament" class="btn btn-primary btn-sm" style="margin-bottom: 10px"><span>THÊM MỚI EVENT</span></button>
                            </div>
                        </div>                                                                 
                        <div class="control-group">
                        ...
                        <div class="control-group">
                            <label class="control-label">Thời gian tính nạp :</label>
                            <div class="controls">
                                <div id="chargingdatestart" name="chargingdatestart" style="display: inline-block"></div>
                                <div id="chargingdateend" name="chargingdateend" style="display: inline-block"></div>
                            </div>
                        </div>

                        <div class="control-group">
                            <label class="control-label">Tổng nạp trong khoảng :</label>
                            <div class="controls">
                                <input name="charging_start" id="charging_start" type="text" class="span3 validate[required,custom[onlyNumberSp]]" /> - 
                                <input name="charging_min" id="charging_min" type="text" class="span3 validate[required,custom[onlyNumberSp]]" />
                            </div>
                        </div>
