...
<div id="content-t" style="min-height: 500px; padding-top: 10px">
        ...
        $(document).ready(function () {
            ...
            $('#onSubmit').on('click', function () {
                ...
                $(".item_rule").each(function (e, item) {
                    var rule = $(item).find("#addVar").attr('keyrule');
                    if (rule != undefined) {
                        rules[rule] = [];
                        $(item).find("#subsub_item_rule.sub_item_rule").each(function (ies, sitem) {
                            rules[rule][ies] = {};
                            rules[rule][ies].item_id = $($(sitem).find("input")[0]).val();
                            rules[rule][ies].name = $($(sitem).find("input")[1]).val();
                            rules[rule][ies].count = $($(sitem).find("input")[2]).val();
                            rules[rule][ies].rate = $($(sitem).find("input")[3]).val();
                            rules[rule][ies].type = $($(sitem).find("input")[4]).val();
                        });
                    }
                });
                ...
                $.ajax({
                    type: "POST",
                    dataType: 'jsonp',
                    url: "<?php echo $url_service; ?>/ToolOnlineNhanQua/<?php echo $_GET['id'] > 0 ? 'edit_' . $_GET['view'] . '?id=' . $_GET['id'] : 'add_' . $_GET['view']; ?>",
                                    data: {
    ...
    <div id="content">
        <!-- Content wrapper -->
        <div class="wrapper">
            <?php include APPPATH . 'views/game/lg/GOK/Events/onlinenhanqua/tab.php'; ?>
            ...
            <div class="widget" id="viewport">
                <form id="frmSendChest" name="frmSendChest" action="" method="POST" onsubmit="return false;" autocomplete="off">
                    <div class="well form-horizontal">
                        ...
                        <div class="control-group" style="display: none;">
                            <label class="control-label" style="display: none;">Loại:</label>
                            <div class="controls">
                                <select name="type">
