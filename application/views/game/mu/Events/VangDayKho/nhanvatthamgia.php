    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            $('#create_boxitem').on('click', function () {
                window.location.href = "/?control=vangdaykho_mu&func=nhanvatthamgia&startdate=" + $("#startdate").val() + "&enddate=" + $("#enddate").val() + "&module=all#nhanvatthamgia";
            });

            $('#export_excel').on('click', function () {
                var start = $("input[name=startdate]").val();
                var end = $("input[name=enddate]").val();
                window.location.href = '/?control=vangdaykho_mu&func=excel_nhanvatthamgia&start=' + start + '&end=' + end+ "&module=all";
            });
            getData();
        });
        ...
        function getData() {
            $("#loading_img").show();
            $.ajax({
                url: "http://game.mobo.vn/mu/cms/toolvangdaykho/get_user_list_join?startdate=" + $("#startdate").val() + "&enddate=" + $("#enddate").val(),
                dataType: 'jsonp',
                method: "POST",
                bRetrieve: true,
                bDestroy: true,
                        ...
                        aoColumns: [
                            ...
                            },
                            {
                                ...
                                mRender: function (data) {
                                    return "<a class='btn btn-success btn-xs' href='/?control=vangdaykho_mu&module=all&func=lichnhanqua&mobo_service_id=" + data + "#nhanvatthamgia'>Lịch Nhận Quà</a>";
                                }
                            }
                        ],
                        bProcessing: true,
