        ...
        <div id="adminfieldset" class="groupsignios">
            ...
            <div class="group_left">
                ...
                <div class="rows">	
                    ...
                    <input type="hidden" name="cbo_app_hiden" id="cbo_app_hiden" value="" />
                            ...
                            <?php
                                if(count($loadgame)>0){
                                    foreach($loadgame as $v){
										if($control['service_id']==$v['service_id']){
                            ?>
                ...
                <div class="rows">	
                    ...
                    <?php echo $control['platform'] ?>
                    <span class="loadapp" style="display:none;">
                ...
                <div class="rows">	
                    ...
                    <select name="cbo_published" id="cbo_published">
                        <option value="0">Chọn tình trạng</option>
                       	    <option value="waiting" <?php echo $views['published']=="waiting"?"selected":""; ?>>Waiting For Publish</option>
                            <option value="yes" <?php echo $views['published']=="yes"?"selected":""; ?>>Yes</option>
                            <option value="no" <?php echo $views['published']=="no"?"selected":""; ?>>No</option>
                    </select>
                    <div class="option_error"></div>
                </div>
            </div>
        ...
        <div id="adminfieldset" class="groupsignios space">
            ...
            <div class="group_left">
                ...
                <div class="rows">	
                    ...
                    <textarea maxlength="1000" name="notes" id="notes" cols="30" rows="5" style="width:500px;height:150px;resize:none;"><?php echo $views['notes']; ?></textarea>
                    
                </div>
				 <div class="rows">	
                    <label for="menu_group_id">Ngày Publish</label>
       <input type="text" id="datepublish" name="datepublish" value="<?php echo $control['datepublish']; ?>" class="datepicker">
                </div>
                <!--<div class="rows">
...
getCert($('#cbo_platform').val());
getBunlderid($('#cbo_type_app').val());
$('#cbo_app_hiden').val($('#cbo_app option:selected').text());
...
maxLength(document.getElementById("notes"));
    ...
    function getCert(_platform){
			...
			var _cert_id=<?php echo $_GET['cert_id']==""?0:$_GET['cert_id']; ?>;
	...
	function getBunlderid(_cert_id){
		...
		var _bunlderid='<?php echo $_GET['bunlderid']==""?0:$_GET['bunlderid']; ?>';
		var _type_app_text=$('#cbo_type_app option:selected').text().split('-');
		$('#cbo_type_app_text').val(_type_app_text[0]);
			$.ajax({
				url:baseUrl+'?control=mestoreversion&func=getbunlderid',
				type:"GET",
				data:{id_game:_id_game,platform:_platform,cert_id:_cert_id,bunlderid:_bunlderid},
				async:false,
				dataType:"json",
				beforeSend:function(){
					$('.loading_warning').show();
