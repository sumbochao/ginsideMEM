...
class MeAPI_Controller_TeThien_Events_TopController implements MeAPI_Controller_TeThien_Events_DotPhaTuong5saoInterface {
    ...
    private $url= "http://game.mobo.vn/tethien/cms/tool_top/";
    ...
    public function index(MeAPI_RequestInterface $request) {
        ...
        $this->CI->template->write_view('content', 'game/tethien/Events/Top/index', $this->data);
    ...
    public function showAddConfig(MeAPI_RequestInterface $request) {
        ...
        if(isset($_GET['end_date']) && !empty($_GET['end_date'])){
            $this->data['end_date'] = $_GET['end_date'];
        }
        ...
        $this->authorize->validateAuthorizeRequest($request, 0);
        $this->CI->template->write_view('content', 'game/tethien/Events/Top/addConfig', $this->data);
        $this->_response = new MeAPI_Response_HTMLResponse($request, $this->CI->template->getHTML());
    }


	public function chienluc(MeAPI_RequestInterface $request){
        $this->authorize->validateAuthorizeRequest($request, 0);
        $this->CI->template->write_view('content', 'game/tethien/Events/Top/chienluc', $this->data);
        $this->_response = new MeAPI_Response_HTMLResponse($request, $this->CI->template->getHTML());
    }
