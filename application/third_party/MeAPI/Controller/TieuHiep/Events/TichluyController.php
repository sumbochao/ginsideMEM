...
class MeAPI_Controller_LK_Events_TichluyController implements MeAPI_Controller_LK_Events_TichluyInterface {
    ...
    //private $url_process = "http://game.mobo.vn/tieuhiep/cms/tichluy/";
    private $url_process = "http://local.service.aow.mobo.vn/cms/tichluy/";
    private $url_link = "http://game.mobo.vn/tieuhiep";
    ...
    public function index(MeAPI_RequestInterface $request) {
        ...
        $this->CI->template->write_view('content', 'game/lk/Events/tichluy/'.$layout.'/index', $this->data);
    ...
    public function add(MeAPI_RequestInterface $request) {
        ...
        $this->CI->template->write_view('content', 'game/lk/Events/tichluy/'.$layout.'/add', $this->data);
        $this->_response = new MeAPI_Response_HTMLResponse($request, $this->CI->template->getHTML());
    }
	function delete(MeAPI_RequestInterface $request){
        ...
        $linkinfo = $this->url_link.'/cms/accumulation_weekly/delete_'.$layout.'?ids='.$ids;
        $infoDetail = file_get_contents($linkinfo);
        $datainfojson = json_decode($infoDetail,true);
        header('location: /?control=accumulation_weekly&func=index&view='.$layout.'&module=all');
    }
	function deleteconfig(MeAPI_RequestInterface $request){
		$this->authorize->validateAuthorizeRequest($request, 0);
        $ids = $request->input_get('ids');
        $linkinfo = $this->url_link.'/cms/accumulation_weekly/delete_config?ids='.$ids;
		
        $infoDetail = file_get_contents($linkinfo);
        $datainfojson = json_decode($infoDetail,true);
        header('location: /?control=accumulation_weekly&func=edit&view=wheel&module=all&ids='.$request->input_get('wheel') );
    }
    ...
    public function edit(MeAPI_RequestInterface $request) {
        ...
        $ids =$request->input_get('ids');
        $linkinfo = $this->url_link.'/cms/accumulation_weekly/getitem_'.$layout.'?ids='.$ids;
        $infoDetail = file_get_contents($linkinfo);
        $datainfojson = json_decode($infoDetail,true);

        $linkinfo = $this->url_link.'/cms/accumulation_weekly/index_item';
    ...
    public function clearcached(MeAPI_RequestInterface $request) {
        $this->authorize->validateAuthorizeRequest($request, 0);
        $this->CI->template->write_view('content', 'game/lk/Events/tichluy/clearcached', $this->data);
        $this->_response = new MeAPI_Response_HTMLResponse($request, $this->CI->template->getHTML());
    }    
