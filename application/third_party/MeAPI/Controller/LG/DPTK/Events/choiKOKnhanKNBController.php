<?php
class MeAPI_Controller_LG_DPTK_Events_choiKOKnhanKNBController implements MeAPI_Controller_LG_DPTK_Events_choiKOKnhanKNBInterface {
    ...
    public function __construct() {
        ...
        $this->url_service = 'https://sev-dptk.addgold.net';
    ...
    public function listserver(MeAPI_RequestInterface $request){
        ...
        $this->data['s_id']= $s_id;
        echo $this->CI->load->view('game/lg/DPTK/Events/choiKOKnhanKNB/filters/listserver', $this->data, true);
        die();
    }
    public function index(MeAPI_RequestInterface $request){
        ...
        switch ($_GET['view']){
            ...
            case 'user':
                ...
                $linkinfoslbEvent = $this->url_service.'/ToolchoiKOKnhanKNB/slbevent';
        ...
        $this->CI->template->write_view('content', 'game/lg/DPTK/Events/choiKOKnhanKNB/'.$_GET['view'].'/index', $this->data);
    ...
    public function add(MeAPI_RequestInterface $request){
        ...
        $this->data['url_service'] = $this->url_service;
        $this->CI->template->write_view('content', 'game/lg/DPTK/Events/choiKOKnhanKNB/'.$_GET['view'].'/add', $this->data);
        $this->_response = new MeAPI_Response_HTMLResponse($request, $this->CI->template->getHTML());
    }
    ...
    public function delete(MeAPI_RequestInterface $request) {
        $this->authorize->validateAuthorizeRequest($request, 0); 
        $id = $_GET['id'];
        $linkInfo = $this->url_service.'/ToolchoiKOKnhanKNB/delete_'.$_GET['view'].'?id='.$id;
    ...
    public function reset_user(MeAPI_RequestInterface $request){
        ...
        $linkInfo = $this->url_service.'/ToolchoiKOKnhanKNB/reset_type?event_id='.$event_id;
        $j_items = file_get_contents($linkInfo);
        redirect(base_url().'?control='.$_GET['control'].'&func=index&view='.$_GET['view'].'&module=all'); 
    }
    public function excel(MeAPI_RequestInterface $request){
        $this->authorize->validateAuthorizeRequest($request, 0);
        $linkInfo = $this->url_service.'/ToolchoiKOKnhanKNB/excel?start='.strtotime($_GET['start']).'&end='.strtotime($_GET['end']);
        $j_items = file_get_contents($linkInfo);
