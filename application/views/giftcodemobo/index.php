...
<form action="" method="post" name="frmindex">
    <div class="rows">
            ...
            if(empty($slbApp) !== TRUE){
                foreach($slbApp as $v){
                    if((@in_array($v['id_app'].'_'.$v['alias_app'], $_SESSION['permission']) && $_SESSION['account']['id_group']==2)|| $_SESSION['account']['id_group']==1){
                        ?>
                        <option value="<?php echo $v['id_app'];?>" <?php echo ($v['id_app']==$items['app'])?'selected="selected"':'';?>><?php echo $v['name_app'];?></option>
                    <?php
                    }
                }
            }
        ...
        <a href="/?control=miniapp&func=addmenu" class="game-button">THÊM EVENT</a>
        <?php echo $error; ?>
    </div>
