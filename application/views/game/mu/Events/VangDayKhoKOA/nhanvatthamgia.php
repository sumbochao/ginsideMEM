...
<div id="content-t" style="min-height: 500px; padding-top: 10px">
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            $('#create_boxitem').on('click', function () {
                window.location.href = "/?control=vangdaykhokoa_mu&func=nhanvatthamgia&startdate=" + $("#startdate").val() + "&enddate=" + $("#enddate").val() + "&module=all#nhanvatthamgia";
            });

            $('#export_excel').on('click', function () {
                var start = $("input[name=startdate]").val();
                var end = $("input[name=enddate]").val();
                window.location.href = '/?control=vangdaykhokoa_mu&func=excel_nhanvatthamgia&start=' + start + '&end=' + end+ "&module=all";
            });
            getData();
        });
        ...
        function getData() {
            $("#loading_img").show();
            $.ajax({
                url: "http://game.mobo.vn/mu/cms/toolvangdaykho_koa/get_user_list_join?startdate=" + $("#startdate").val() + "&enddate=" + $("#enddate").val(),
                dataType: 'jsonp',
                method: "POST",
                bRetrieve: true,
                bDestroy: true,
                        ...
                        aoColumns: [
                            ...
                            },
                            {
                                ...
                                mRender: function (data) {
                                    return "<a class='btn btn-success btn-xs' href='/?control=vangdaykhokoa_mu&module=all&func=lichnhanqua&mobo_service_id=" + data + "#nhanvatthamgia'>Lịch Nhận Quà</a>";
                                }
                            }
                        ],
                        bProcessing: true,
    ...
    <div id="content">
        <!-- Content wrapper -->
        <div class="wrapper">
            <?php include APPPATH . 'views/game/mu/Events/VangDayKhoKOA/tab.php'; ?>
