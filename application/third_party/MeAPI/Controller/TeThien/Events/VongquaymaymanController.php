<?php
class MeAPI_Controller_TeThien_Events_VongquaymaymanController implements MeAPI_Controller_TeThien_Events_VongquaymaymanInterface {
    protected $_response;
    private $CI;
    private $url_service;
    ...
    public function __construct() {
        ...
        if($_SERVER['SERVER_ADDR']=='127.0.0.1'){
            $this->url_service = 'http://localhost.service.tethien.mobo.vn';
        }else{
            $this->url_service = 'http://game.mobo.vn/tethien';
        }
    }
    public function index(MeAPI_RequestInterface $request){
        $this->authorize->validateAuthorizeRequest($request, 0);
         $this->data['title']= 'DANH SÁCH VÒNG QUAY MAY MẮN';
        $this->CI->template->write_view('content', 'game/tethien/Events/vongquaymayman/index', $this->data);
        $this->_response = new MeAPI_Response_HTMLResponse($request, $this->CI->template->getHTML());
    }
    public function add(MeAPI_RequestInterface $request){
        $this->data['title']= 'THÊM VÒNG QUAY MAY MẮN';
        $this->authorize->validateAuthorizeRequest($request, 0);     
        $this->CI->template->write_view('content', 'game/tethien/Events/vongquaymayman/add', $this->data);
        $this->_response = new MeAPI_Response_HTMLResponse($request, $this->CI->template->getHTML());
    }
    public function edit(MeAPI_RequestInterface $request){
        $this->authorize->validateAuthorizeRequest($request, 0);
        $this->data['title']= 'SỬA VÒNG QUAY MAY MẮN';
        $id = $_GET['id'];
        $linkinfo = $this->url_service.'/cms/vongquaymayman/gettem?id='.$id;
    ...
    public function delete(MeAPI_RequestInterface $request) {
        $this->authorize->validateAuthorizeRequest($request, 0); 
        $id = $_GET['id'];
        $linkInfo = $this->url_service.'/cms/vongquaymayman/delete?id='.$id;
    ...
    public function history(MeAPI_RequestInterface $request){
        $this->authorize->validateAuthorizeRequest($request, 0);
        $this->data['title']= 'LỊCH SỬ VÒNG QUAY MAY MẮN';
        $this->CI->template->write_view('content', 'game/tethien/Events/vongquaymayman/history', $this->data);
        $this->_response = new MeAPI_Response_HTMLResponse($request, $this->CI->template->getHTML());
    }
    public function excel(MeAPI_RequestInterface $request){
        $this->authorize->validateAuthorizeRequest($request, 0);
        $linkInfo = $this->url_service.'/cms/vongquaymayman/excel?start='.strtotime($_GET['start']).'&end='.strtotime($_GET['end']);
        $j_items = file_get_contents($linkInfo);
