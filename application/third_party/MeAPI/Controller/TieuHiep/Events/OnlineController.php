...
class MeAPI_Controller_TieuHiep_Events_OnlineController implements MeAPI_Controller_TieuHiep_Events_OnlineInterface {
    ...
    public $url_link = "http://game.mobo.vn/tieuhiep";
    ...
    public function index(MeAPI_RequestInterface $request) {
        ...
        $this->CI->template->write_view('content', 'game/tieuhiep/Events/quaonline/'.$layout.'/index', $this->data);
    ...
    public function add(MeAPI_RequestInterface $request) {
        ...
        $this->CI->template->write_view('content', 'game/tieuhiep/Events/quaonline/'.$layout.'/add', $this->data);
        $this->_response = new MeAPI_Response_HTMLResponse($request, $this->CI->template->getHTML());
    }
	function delete(MeAPI_RequestInterface $request){
        ...
        $linkinfo = $this->url_link.'/cms/quaonline/delete_'.$layout.'?ids='.$ids;
        $infoDetail = file_get_contents($linkinfo);
        $datainfojson = json_decode($infoDetail,true);
        header('location: /?control=quaonline&func=index&view='.$layout);
    }
	function deleteconfig(MeAPI_RequestInterface $request){
		$this->authorize->validateAuthorizeRequest($request, 0);
        $ids = $request->input_get('ids');
        $linkinfo = $this->url_link.'/cms/quaonline/delete_config?ids='.$ids;
		
        $infoDetail = file_get_contents($linkinfo);
        $datainfojson = json_decode($infoDetail,true);
        header('location: /?control=quaonline&func=edit&view=wheel&module=all&ids='.$request->input_get('wheel') );
    }
    ...
    public function edit(MeAPI_RequestInterface $request) {
        ...
        $ids =$request->input_get('ids');
        $linkinfo = $this->url_link.'/cms/quaonline/getitem_'.$layout.'?ids='.$ids;
        $infoDetail = file_get_contents($linkinfo);
        $datainfojson = json_decode($infoDetail,true);

        $linkinfo = $this->url_link.'/cms/quaonline/index_item';
    ...
    public function clearcached(MeAPI_RequestInterface $request) {
        $this->authorize->validateAuthorizeRequest($request, 0);
        $this->CI->template->write_view('content', 'game/tieuhiep/Events/quaonline/clearcached', $this->data);
        $this->_response = new MeAPI_Response_HTMLResponse($request, $this->CI->template->getHTML());
    }    
