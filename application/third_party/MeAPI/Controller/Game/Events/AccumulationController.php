<?php
class MeAPI_Controller_Game_Events_AccumulationController implements MeAPI_Controller_Game_Events_AccumulationInterface {
    protected $_response;
    private $CI;
    private $url_service;
    ...
    public function __construct() {
		...
		switch ($_GET['game']){
            ...
            case '130':
                if($_SERVER['SERVER_ADDR']=='127.0.0.1'){
                    $this->url_service = 'http://localhost.tieuhiep.mobo.vn';
                }else{
			...
			case '154':
                if($_SERVER['SERVER_ADDR']=='127.0.0.1'){
                    $this->url_service = 'http://localhost.service.hero.mobo.vn';
                }else{
			...
			case '152':
                if($_SERVER['SERVER_ADDR']=='127.0.0.1'){
                    $this->url_service = 'http://localhost.service.3q.mobo.vn';
                }else{
    ...
    public function index(MeAPI_RequestInterface $request){
        ...
        $this->CI->template->write_view('content', 'game/Events/accumulation/'.$_GET['view'].'/index', $this->data);
    ...
    public function add(MeAPI_RequestInterface $request){
        ...
        $this->CI->template->write_view('content', 'game/Events/accumulation/'.$_GET['view'].'/add', $this->data);
        $this->_response = new MeAPI_Response_HTMLResponse($request, $this->CI->template->getHTML());
    }
    ...
    public function delete(MeAPI_RequestInterface $request) {
		...
		if($_GET['view']=='history'){
            redirect(base_url().'?control='.$_GET['control'].'&func=history&module=all&game='.$_GET['game']); 
        }else{
    ...
    public function history(MeAPI_RequestInterface $request){
        ...
        $this->CI->template->write_view('content', 'game/Events/accumulation/history', $this->data);
        $this->_response = new MeAPI_Response_HTMLResponse($request, $this->CI->template->getHTML());
    }
