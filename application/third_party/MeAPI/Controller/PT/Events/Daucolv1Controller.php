...
class MeAPI_Controller_PT_Events_Daucolv1Controller implements MeAPI_Controller_PT_Events_Daucolv1Interface {
    ...
    private $url_process = "http://game.mobo.vn/phongthan/cms/tooldaucolv1/";
    ...
    public function index(MeAPI_RequestInterface $request) {
        ...
        $this->CI->template->write_view('content', 'game/pt/Events/DauCoLV1/index', $this->data);
    ...
    public function quanlyqua(MeAPI_RequestInterface $request) {
        $this->authorize->validateAuthorizeRequest($request, 0);
        $this->CI->template->write_view('content', 'game/pt/Events/DauCoLV1/quanlyqua', $this->data);
        $this->_response = new MeAPI_Response_HTMLResponse($request, $this->CI->template->getHTML());
    }
    ...
    public function themqua(MeAPI_RequestInterface $request) {
        ...
        $this->authorize->validateAuthorizeRequest($request, 0);
        $this->CI->template->write_view('content', 'game/pt/Events/DauCoLV1/themqua', $this->data);
        $this->_response = new MeAPI_Response_HTMLResponse($request, $this->CI->template->getHTML());
    }
    ...
    public function lichsu(MeAPI_RequestInterface $request){
        $this->authorize->validateAuthorizeRequest($request, 0);
        $this->CI->template->write_view('content', 'game/pt/Events/DauCoLV1/lichsu', $this->data);
        $this->_response = new MeAPI_Response_HTMLResponse($request, $this->CI->template->getHTML());
    }

	
	public function packages(MeAPI_RequestInterface $request){
        $this->authorize->validateAuthorizeRequest($request, 0);
        $this->CI->template->write_view('content', 'game/pt/Events/DauCoLV1/packages', $this->data);
        $this->_response = new MeAPI_Response_HTMLResponse($request, $this->CI->template->getHTML());
    }
	
	
    public function config_event(MeAPI_RequestInterface $request){
        $result = $this->curlPostAPI(array('status' => $_POST['status'],'server_list' => $_POST['server_list']), $this->url_process."config_event");
        $this->_response = new MeAPI_Response_HTMLResponse($request, json_encode($result));
    }
