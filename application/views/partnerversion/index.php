...
<div id="content-t" class="content_form" style="min-height:500px; padding-top:10px">
    ...
    <form id="appForm" action="" method="post" name="appForm">
        ...
        <div class="filter">
            ...
            <select name="cbo_partner" id="cbo_partner" class="chosen-select" tabindex="2" data-placeholder="Chọn game">
                <option value="0">Chọn đối tác</option>
                <?php
                    if(count($loadpartner)>0){
                        foreach($loadpartner as $v){
                ?>
                <option value="<?php echo $v['id'];?>" <?php echo ($_POST['cbo_partner']==$v['id']) || ($_GET['cbo_partner']==$v['id'])?'selected="selected"':'';?>><?php echo $v['partner'];?></option>
                <?php
                        }
                    }
                ?>
            ...
            <select name="cbo_app" id="cbo_app" class="chosen-select" tabindex="2" data-placeholder="Chọn game">
                ...
                ?>
                <option value="<?php echo $v['service_id'];?>" <?php echo ($_POST['cbo_app']==$v['service_id']) || ($_GET['service_id']==$v['service_id'])?'selected="selected"':'';?>><?php echo $v['app_fullname'];?></option>
                <?php
                        }
                    }
                ?>
        ...
        <table width="100%" border="0" class="table table-striped table-bordered table-condensed table-hover" id="tbl_viewdata">
            ...
            <tr id="rows_<?php echo $i;?>" class="rows_class">
					...
					?>
                    <td><a href="javascript:void(0);" onclick="onSubmitForm('appForm','<?php echo base_url()."?control=".$_GET['control']."&func=edit&service_id=".$rowdb['data']['service_id']."&msv_id=".$rowdb['data']['msv_id']."&platform=".$rowdb['data']['platform']."&status=".$rowdb['data']['status']."&type_app=".$result1[0]['type_app']."&published=".$result1[0]['published']."&id=".$rowdb['data']['id']."&notes=".base64_encode($result1[0]['notes']==""?"":$result1[0]['notes'])."&mebutton=".$rowdb['data']['me_button']."&mechat=".$rowdb['data']['me_chat']."&meevent=".$rowdb['data']['me_event']."&megame=".$rowdb['data']['me_game']."&melogin=".$rowdb['data']['me_login']."&memsg=".base64_encode($rowdb['data']['msg_login']==""?"{\"link\":\"\",\"message\":\"\"}":$rowdb['data']['msg_login']); ?>')">Edit Msv</a> | 
                    ...
                    <a href="<?=base_url()."?control=popupbutton&func=index&msv_id=".$rowdb['data']['msv_id']."&platform=".$rowdb['data']['platform']."&status=".$rowdb['data']['status']."&service_id=".$rowdb['data']['service_id']."&id=".$rowdb['data']['id']."&mebutton=".$rowdb['data']['me_button']."&mechat=".$rowdb['data']['me_chat']."&meevent=".$rowdb['data']['me_event']."&megame=".$rowdb['data']['me_game']."&melogin=".$rowdb['data']['me_login']."&megm=".$rowdb['data']['me_gm']."&memsg=".base64_encode($rowdb['data']['msg_login']==""?"{\"link\":\"\",\"message\":\"\"}":$rowdb['data']['msg_login'])."&iframe=true&width=330&height=330";?>" rel="prettyPhoto2[iframes]" >Edit Me button</a>
                    </td>
                    <td><?php echo $i; ?></td>
                    ...
                    <td id="td_<?php echo $i; ?>"><?php echo $result1[0]['published']; ?></td>
