<?php
class MeAPI_Controller_DHT_Events_BaodanhController implements MeAPI_Controller_DHT_Events_BaodanhInterface {
    protected $_response;
    private $CI;
    private $url_service;
    ...
    public function __construct() {
        ...
        if($_SERVER['SERVER_ADDR']=='127.0.0.1'){
            $this->url_service = 'http://localhost.service.onepiece.mobo.vn/onepiece/index.php';
        }else{
            $this->url_service = 'http://game.mobo.vn/onepiece';
        }
    ...
    public function listserver(MeAPI_RequestInterface $request){
        ...
        $this->data['s_id']= $s_id;
        echo $this->CI->load->view('game/dht/Events/baodanh/filters/listserver', $this->data, true);
        die();
    }
    public function index(MeAPI_RequestInterface $request){
        ...
        $this->CI->template->write_view('content', 'game/dht/Events/baodanh/'.$_GET['view'].'/index', $this->data);
    ...
    public function add(MeAPI_RequestInterface $request){
        ...
        switch ($_GET['view']){
            ...
            case 'filters':
                ...
                $linkinfoslbEvent = $this->url_service.'/cms/baodanh/slbevent';
        ...
        $this->data['url_service'] = $this->url_service;
        $this->CI->template->write_view('content', 'game/dht/Events/baodanh/'.$_GET['view'].'/add', $this->data);
        $this->_response = new MeAPI_Response_HTMLResponse($request, $this->CI->template->getHTML());
    }
    public function edit(MeAPI_RequestInterface $request){
        ...
        $linkinfo = $this->url_service.'/cms/baodanh/get_'.$_GET['view'].'?id='.$id;
    ...
    public function delete(MeAPI_RequestInterface $request) {
        $this->authorize->validateAuthorizeRequest($request, 0); 
        $id = $_GET['id'];
        $linkInfo = $this->url_service.'/cms/baodanh/delete_'.$_GET['view'].'?id='.$id;
    ...
    public function excel(MeAPI_RequestInterface $request){
        $this->authorize->validateAuthorizeRequest($request, 0);
        $linkInfo = $this->url_service.'/cms/baodanh/excel?start='.$_GET['start'].'&end='.$_GET['end'];
        $j_items = file_get_contents($linkInfo);
    ...
    public function excel_baodanh(MeAPI_RequestInterface $request){
        $this->authorize->validateAuthorizeRequest($request, 0);
        $linkInfo = $this->url_service.'/cms/baodanh/excel_baodanh?start='.$_GET['start'].'&end='.$_GET['end'];
        $j_items = file_get_contents($linkInfo);
        $listItem = json_decode($j_items,true);
        
        header('Content-type: application/excel');
        $filename = 'bao_danh_tham_gia.xls';
        header('Content-Disposition: attachment; filename='.$filename);
        $data .= '<html xmlns:x="urn:schemas-microsoft-com:office:excel">
        <head>
            <meta http-equiv="Content-Type" content="text/html; charset=UTF-8" />
        ...
        if(count($listItem)){
            foreach($listItem as $v){
                $date = date_format(date_create($v['date']),"d-m-Y H:i:s");
                switch ($v['type']){
                    case '1':
                        $type = 'Liên tục';
                        break;
