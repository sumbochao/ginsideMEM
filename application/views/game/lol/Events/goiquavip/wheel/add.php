...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        $('#comeback').on('click',function(){
            window.location.href ='?control=lol_event_goi_qua_vip&func=index&view=wheel&module=all';
        });
        ...
        $('#onSubmit').on('click',function(){
            ...
            $.ajax({
                type: "POST",
                dataType: 'jsonp',
                url: "<?php echo $url; ?><?php echo ($_GET['ids']>0)?'edit_wheel?ids='.$_GET['ids']:'add_wheel';?>",
...
<div id="content">
    ...
    <div class="wrapper">
        ...
        $end = date('Y-m-d H:i:s',time()+7*3600);
        if($_GET['ids']>0){
									...
									<select id="game" name="game" class="span4 " />
										...
										if($listgame) {
											foreach ($listgame as $key => $val) {
												if ($val['alias'] == $infodetail[0]['game']) {
