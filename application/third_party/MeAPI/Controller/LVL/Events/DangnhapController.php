<?php
class MeAPI_Controller_LVL_Events_DangnhapController implements MeAPI_Controller_LVL_Events_DangnhapInterface {
    protected $_response;
    private $CI;
    private $url_service;
    ...
    public function listserver(MeAPI_RequestInterface $request){
        ...
        $this->data['s_id']= $s_id;
        echo $this->CI->load->view('game/lvl/Events/dangnhap/filters/listserver', $this->data, true);
        die();
    }
    public function index(MeAPI_RequestInterface $request){
        ...
        switch ($_GET['view']){
            ...
            case 'user':
                ...
                $linkinfoslbEvent = $this->url_service.'/cms/dangnhap/slbevent';
        ...
        $this->CI->template->write_view('content', 'game/lvl/Events/dangnhap/'.$_GET['view'].'/index', $this->data);
    ...
    public function add(MeAPI_RequestInterface $request){
        ...
        $this->data['url_service'] = $this->url_service;
        $this->CI->template->write_view('content', 'game/lvl/Events/dangnhap/'.$_GET['view'].'/add', $this->data);
        $this->_response = new MeAPI_Response_HTMLResponse($request, $this->CI->template->getHTML());
    }
    public function edit(MeAPI_RequestInterface $request){
        ...
        $linkinfo = $this->url_service.'/cms/dangnhap/get_'.$_GET['view'].'?id='.$id;
    ...
    public function delete(MeAPI_RequestInterface $request) {
        $this->authorize->validateAuthorizeRequest($request, 0); 
        $id = $_GET['id'];
        $linkInfo = $this->url_service.'/cms/dangnhap/delete_'.$_GET['view'].'?id='.$id;
    ...
    public function reset_user(MeAPI_RequestInterface $request){
        ...
        $linkInfo = $this->url_service.'/cms/dangnhap/reset_type?event_id='.$event_id;
        $j_items = file_get_contents($linkInfo);
        redirect(base_url().'?control='.$_GET['control'].'&func=index&view='.$_GET['view'].'&module=all'); 
    }
    public function excel(MeAPI_RequestInterface $request){
        $this->authorize->validateAuthorizeRequest($request, 0);
        $linkInfo = $this->url_service.'/cms/dangnhap/excel?start='.strtotime($_GET['start']).'&end='.strtotime($_GET['end']);
        $j_items = file_get_contents($linkInfo);
