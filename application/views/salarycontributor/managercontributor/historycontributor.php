    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            $('#create_boxitem').on('click', function () {
                window.location.href = "/?control=managercontributor&func=historycontributor&startdate=" + $("#startdate").val() + "&enddate=" + $("#enddate").val() + "#lichsu";
            });
...
//                url: "http://mem.net.vn/cms/toolsalarycollaborator/games_list",
        ...
        function getData(gamelistId) {
            $("#loading_img").show();
            $.ajax({
                url: "http://mem.net.vn/cms/toolsalarycollaborator/get_historycontributor?startdate=" + $("#startdate").val() + "&enddate=" + $("#enddate").val() + "&gamelistId=" + gamelistId + "&pagesize=10000",
                dataType: 'jsonp',
                method: "POST",
                bRetrieve: true,
                bDestroy: true,
                        ...
                        aoColumns: [
                            ...
                            },
                            {
                                ...
                                mRender: function (data) {
                                    if (data == 0) {
                                        return "<span style='color:green'>Lương</span>";
                                    } else if (data == 1) {
                                        return "<span style='color:blue'>Đổi Item</span>";
                                    } else if (data == 2) {
                                        return "<span style='color:orange'>Cho tặng</span>";
                                    } else if (data == 3) {
                                ...
                                mRender: function (data, type, full) {
                                    ...
                                    if (obj2 !== null) {
                                        $.each(obj2, function (key2, value2) {
                                            if (key2 == "result") {
                                                if (value2) {
                                                    html = html + "Nhận : Thành Công " + "</br>";
                                                } else {
                                                    html = html + "Nhận : Thất Bại " + "</br>";
                                                }
                                            } else if (Object.keys(value2).indexOf("giftcode") == -1) {
                                                html = html + "Item Name : " + value2[0]["item_name"] + " - Số lượng : " + value2[0]["count"] + "</br>";
                                            } else {
                                                html = html + "Giftcode : " + value2["giftcode"] + "</br>";
                                            }
                                        });
                                        return html;
                                    }
