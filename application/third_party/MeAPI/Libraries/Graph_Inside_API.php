...
class Graph_Inside_API {
    ...
    private $url_gsv = 'http://misc.dllglobal.net';
	...
	public function checkgamepartner($url_param){
        ...
        public function view_gsv($url_param){
			$strURL = $this->url_gsv ."/?". $url_param;
	...
	public function gsv_add($arrParam){
		$result = $this->_call_api_gsv($arrParam);
		if (is_array($result) == TRUE) {
    ...
    public function gsv_update_button($arrParam) {
        $result = $this->_call_api_gsv($arrParam);
		//var_dump($result);
        if (is_array($result) == TRUE) {
    ...
    private function _call_api_gsv($params) {
        ...
        $link = $this->url_gsv . "/?" . http_build_query($params);
