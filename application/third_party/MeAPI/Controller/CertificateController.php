<?php
class MeAPI_Controller_CertificateController{
    protected $_response;
    private $CI;
    private $_mainAction;
	...
	public function noaccess(MeAPI_RequestInterface $request){
        ...
        $this->CI->template->write_view('content', 'certificate/noaccess', $this->data);
    ...
    public function index(MeAPI_RequestInterface $request) {
        ...
        $listItems = $this->CI->CertificateModel->listItem($arrFilter);
		...
		$this->data['lblUser'] = $this->CI->CertificateModel->listUser();
		$this->data['lblPartner'] = $this->CI->PartnerModel->listPartner();
		
        $this->CI->template->write_view('content', 'certificate/index', $this->data);
        $this->_response = new MeAPI_Response_HTMLResponse($request, $this->CI->template->getHTML());
    }
    ...
    public function edit(MeAPI_RequestInterface $request){
        ...
        if (isset($get['action']) && $get['action']==1){
			...
			$rs=$this->CI->CertificateModel->saveItem($arrControl,array("task"=>"edit"));	
			if($rs){
		...
		$this->data['items'] = $this->CI->CertificateModel->getItem((int)$_GET['id']);
		$this->data['partner'] = $this->CI->PartnerModel->listItem();
        $this->CI->template->write_view('content', 'certificate/edit', $this->data);
        $this->_response = new MeAPI_Response_HTMLResponse($request, $this->CI->template->getHTML());
    }
    public function add(MeAPI_RequestInterface $request){
        ...
        if (isset($get['action']) && $get['action']==1){
			...
			$rs=$this->CI->CertificateModel->saveItem($arrControl,array("task"=>"add"));	
			if($rs){
        ...
        $this->CI->template->write_view('content', 'certificate/add', $this->data);
        $this->_response = new MeAPI_Response_HTMLResponse($request, $this->CI->template->getHTML());
    }
    public function sort(MeAPI_RequestInterface $request){
        $arrParam = array_merge($_POST,$_GET);
        if ($this->CI->input->post()){
            $this->CI->CertificateModel->sortItem($arrParam);
        }
    ...
    public function status(MeAPI_RequestInterface $request){
        ...
        if($arrParam['type']=='multi'){
            $this->CI->CertificateModel->statusItem($arrParam,array('task'=>'multi'));
        }else{
            $this->CI->CertificateModel->statusItem($arrParam);
        }
        redirect($this->_mainAction);
    }
    public function delete(MeAPI_RequestInterface $request){
        $arrParam = array_merge($_POST,$_GET);
        if($arrParam['type']=='multi'){
            $this->CI->CertificateModel->deleteItem($arrParam,array('task'=>'multi'));
        }else{
            $this->CI->CertificateModel->deleteItem($arrParam);
        }
        redirect($this->_mainAction);
    }
