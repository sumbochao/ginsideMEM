...
//Load Gift VIP Details
        function get_item(id) {
            $.ajax({
                method: "GET",
                url: "http://mem.net.vn/cms/toolsalarycollaborator/get_item_detail?id=" + id,
                contentType: 'application/json; charset=utf-8',
                success: function (data) {
                    //Load Reward Details  
                    $("#id").val(data["id"]);
                    ...
                    if (data["type"] == 1) {
                        $("#giftcode_type").prop("checked", true);
                    } else {
                        $("#item_type").prop("checked", true);
                    }
                    $("#item_count").val(data["item_count"]);
                    $("#item_rate").val(data["item_rate"]);
                    $("#limit_buy_user").val(data["limit_buy_user"]);
                    $("#price").val(data["price"]);
                    $("#gift_img").attr('src', data["icon"]);
                    $("#gift_img_text").val(data["icon"]);

                    if (data["status"] != 2) {
                        $('#item_enable').val(data["status"]);
                        $('#item_enable').prop('checked', true);
                    } else {
                        $('#item_disable').prop('checked', true);
                    }
                    var item_elements = $.parseJSON(data["item_elements"]);
                    key = (Object.keys(item_elements[0]));
                    len = key.length;
                    if (len > 1) {
                    }
                    ...
                    for (var i = 0; i < len; ++i) {
                        ...
                        if (i > 0) {
                            $(".add_more_item").before(
                                    ...
                                    + '<div class="col-md-6">'
                                    + '<input name="type1[' + i + ']" id="type1_' + i + '" type="text" class="span3 validate[required] form-control" />'
                                    + '</div>'
                                    + '<div class="col-md-6" >'
                                    + '<input name="type2[' + i + ']" id="type2_' + i + '" type="text" class="span3 validate[required] form-control" />'
                                    + '</div>'
                                    + '</div>'
                                    + '<a class="minus-element">-</a>'
                                    + '</div>');
                            $("#type1_" + i).val(key[i]);
                            $("#type2_" + i).val(item_elements[0][key[i]]);
                        } else {
                            $("#type1_" + i).val(key[i]);
                            $("#type2_" + i).val(item_elements[0][key[i]]);
                        }
                        $("#type1_" + i).easyAutocomplete(options);
                    }
                },
                error: function (data) {
                    var obj = $.parseJSON(data);
...
//        $(document).ready(function () {
//            var options = {
//                data: ["item_id", "item_name", "item_type", "position", "activity"],
//            };
    ...
    <div id="content">
        ...
        <div class="wrapper">
            ...
            <div class="widget" id="viewport">
                <form id="frmSendChest" name="frmSendChest" action="/?control=managercontributor&func=edititem" method="POST" enctype="multipart/form-data">
                    <div class="well form-horizontal">
                        ...
                        <div class="control-group">
                            <label class="control-label">Cập nhật Icon:</label>
                            <div class="controls">
                        ...
                        <div class="control-group">
                            ...
                            <div class="controls">
                                Enable:<input type="radio" name="status" id="item_enable" value="" checked="">
                                &nbsp;&nbsp;
                                Disable:<input type="radio" name="status" id="item_disable" value="2">
                            </div>
                        </div>
                        ...
                        <div class="control-group">
                            <div style="padding-left: 20%; text-align: left;">
                                ...
                                <input type="hidden" name='updated' value="<?php echo $_SESSION['account']['username']; ?>">
