...
<div id="content-t" style="min-height: 500px; padding-top: 10px">
    ...
    <script type="text/javascript">
        $(function () {
            $("#tournament_id").change(function () {
                //getData($(this).val());
                window.location.href = "/?control=event_covu_lvl&module=all&func=lichsu&tournament_id=" + $(this).val() + "&startdate=" + $("#startdate").val() + "&enddate=" + $("#enddate").val() + "#lichsu";
            });
        });
        ...
        $(document).ready(function () {
            ...
            $('#create_boxitem').on('click', function () {
                window.location.href = "/?control=event_covu_lvl&module=all&func=lichsu&tournament_id=" + $("#tournament_id").val() + "&startdate=" + $("#startdate").val() + "&enddate=" + $("#enddate").val() + "#lichsu";
            });

            $('#export_excel').on('click', function () {
                window.location.href = "http://game.mobo.vn/volam/cms/toolcacuoc/export_excel_pet_history?tournament_id=" + $("#tournament_id").val() +"&startdate=" + $("#startdate").val() + "&enddate=" + $("#enddate").val();
            });
            ...
            $.ajax({
                method: "GET",
                url: "http://game.mobo.vn/volam/cms/toolcacuoc/tournament_list",
        ...
        function getData(tournament_id) {
            $("#loading_img").show();
            $.ajax({
                url: "http://game.mobo.vn/volam/cms/toolcacuoc/get_pet_history_details_by_tournament?id=" + tournament_id + "&startdate=" + $("#startdate").val() + "&enddate=" + $("#enddate").val() + "&pagesize=10000",
                dataType: 'jsonp',
                method: "POST",
                bRetrieve: true,
                bDestroy: true,
                        ...
                        aoColumns: [
                                ...
                                },
                                {
                                 ...
                                 },                                 
                                 {
                                     ...
                                     mData: "pet_status",
                                     mRender: function (data) {
                                         return (data === "wait") ? "<span style='color: #f79646'>Chờ kết quả...</span>" : (data == "win") ? "<span style='color: blue'>Thắng</span>" : (data == "draw") ? "<span style='color: #f36926'>Hòa</span>" : "<span style='color: red'>Thua</span>";
                                     }
                                 },
                                 {
                                     sTitle: "Điểm thắng",
    ...
    <div id="content">
        <!-- Content wrapper -->
        <div class="wrapper">
            <?php include APPPATH . 'views/game/lvl/Events/CaCuoc/tab.php'; ?>
