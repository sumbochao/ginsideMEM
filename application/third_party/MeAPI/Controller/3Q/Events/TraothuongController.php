<?php
class MeAPI_Controller_3Q_Events_TraothuongController implements MeAPI_Controller_3Q_Events_TraothuongInterface {
    protected $_response;
    private $CI;
    private $url_service;
    ...
    public function __construct() {
        ...
        $this->CI->load->MeAPI_Model('Service/3Q/TraothuongModel');
    ...
    public function index(MeAPI_RequestInterface $request){
        ...
        $this->CI->template->write_view('content', 'game/3q/Events/traothuong/'.$_GET['view'].'/index', $this->data);
    ...
    public function add(MeAPI_RequestInterface $request){
        ...
        $this->data['url_service'] = $this->url_service;
        $this->CI->template->write_view('content', 'game/3q/Events/traothuong/'.$_GET['view'].'/add', $this->data);
        $this->_response = new MeAPI_Response_HTMLResponse($request, $this->CI->template->getHTML());
    }
    public function edit(MeAPI_RequestInterface $request){
        ...
        $linkinfo = $this->url_service.'/cms/traothuong/get_'.$_GET['view'].'?id='.$id;
    ...
    public function import(MeAPI_RequestInterface $request) {
        ...
        if ($this->CI->input->post()){
            ...
            if(!in_array($file[1],array('xls'))){
                ...
                $data_excel = array();
                for ($i=2;$i<=$rowsnum;$i++){
                    $data_excel[] = array('msi'=>$excel->val($i,1),'type'=>$excel->val($i,2),'item'=>$excel->val($i,3));
                }
                $this->CI->TraothuongModel->saveItem($data_excel);
                $this->data['success'] = 'Import thành công';
            }
        }
        $this->CI->template->write_view('content', 'game/3q/Events/traothuong/'.$_GET['view'].'/import', $this->data);
        $this->_response = new MeAPI_Response_HTMLResponse($request, $this->CI->template->getHTML());
    }
    public function delete(MeAPI_RequestInterface $request) {
        $this->authorize->validateAuthorizeRequest($request, 0); 
        $id = $_GET['id'];
        $linkInfo = $this->url_service.'/cms/traothuong/delete_'.$_GET['view'].'?id='.$id;
    ...
    public function history(MeAPI_RequestInterface $request){
        ...
        $this->data['url_service'] = $this->url_service;
        $this->CI->template->write_view('content', 'game/3q/Events/traothuong/history', $this->data);
        $this->_response = new MeAPI_Response_HTMLResponse($request, $this->CI->template->getHTML());
    }
    public function excel(MeAPI_RequestInterface $request){
        $this->authorize->validateAuthorizeRequest($request, 0);
        $linkInfo = $this->url_service.'/cms/traothuong/excel?start='.$_GET['start'].'&end='.$_GET['end'];
        $j_items = file_get_contents($linkInfo);
        $listItem = json_decode($j_items,true);
        
        header('Content-type: application/excel');
        $filename = 'trao_thuong.xls';
        ...
        if(count($listItem)){
            foreach($listItem as $v){
                ...
                $data .= '<tr>
                    ...
                    <td align="center">Gói '.$v['goiqua'].'</td>
